...
const DATABASE_URL = process.env.DATABASE_URL ?? "postgres://metanicator:metanicator@localhost:5432/metanicator?sslmode=disable";
...
export class TestApiClient {
  ...
  async login(email: string, name: string) {
    ...
    try {
      ...
      const configuredDevCode = process.env.METANICATOR_DEV_VERIFICATION_CODE?.trim();
