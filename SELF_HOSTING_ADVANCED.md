...
This document covers advanced configuration for self-hosted Metanicator deployments. For the quick start guide, see [SELF_HOSTING.md](SELF_HOSTING.md).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string | `postgres://metanicator:metanicator@localhost:5432/metanicator?sslmode=disable` |
...
Metanicator supports two email backends. `SMTP_HOST` takes priority when set; otherwise `RESEND_API_KEY` is used. With neither configured, verification codes are printed to the server log — copy them from there to log in.
...
| `RESEND_FROM_EMAIL` | Sender email address (default: `noreply@metanicator.ai`) |
...
> **Note:** If neither Resend nor SMTP is configured, generated verification codes are printed to backend logs — copy them from there to log in. A fixed local testing code (e.g. `888888`) is **opt-in only**: set `METANICATOR_DEV_VERIFICATION_CODE=888888` in `.env` and keep `APP_ENV` non-production. The Docker self-host stack pins `APP_ENV=production`, so the shortcut is ignored there. **Never enable a fixed code on a publicly reachable instance.**
...
If the frontend and backend are served from different hostnames, `COOKIE_DOMAIN` is **required**, not optional: the browser must be able to read the `metanicator_csrf` cookie from the page's own origin to send the `X-CSRF-Token` header, and without it every write request fails with `403 {"error":"CSRF validation failed"}`. Scope it to the narrowest parent domain that covers both hosts — it also shares the `metanicator_auth` session cookie with every host under that domain. See [Reverse Proxy](#reverse-proxy) for the full split-domain configuration and its trust requirements.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `METANICATOR_SERVER_URL` | `ws://localhost:8080/ws` | WebSocket URL for daemon → server connection |
| `METANICATOR_APP_URL` | `http://localhost:3000` | Frontend URL for CLI login flow |
| `METANICATOR_DAEMON_POLL_INTERVAL` | `3s` | How often the daemon polls for tasks |
| `METANICATOR_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat frequency |
...
| `METANICATOR_COPILOT_PATH` | Custom path to the `copilot` (GitHub Copilot CLI) binary |
...
| `METANICATOR_GROK_PATH` | Custom path to the `grok` binary |
| `METANICATOR_GROK_MODEL` | Override the Grok model used (e.g. `grok-4.5`) |

## Database Setup

Metanicator requires PostgreSQL 17 with the pgvector extension.
...
# Kubernetes
kubectl -n metanicator exec deploy/metanicator-backend -- \
  ./backfill_task_usage_hourly --sleep-between-slices=2s
```
...
metanicator.example.com {
    ...
    @metanicator_ws path /ws /ws/*
    handle @metanicator_ws {
...
By default, Metanicator works on `localhost`. If you access it from another machine on the LAN (e.g. `http://192.168.1.100:3000`), you need to tell the backend to accept that origin:
...
Pin `METANICATOR_IMAGE_TAG` in `.env` to an exact release like `v0.2.4` if you want to stay on a specific version. Migrations run automatically on backend startup. They are idempotent — running them multiple times has no effect.
