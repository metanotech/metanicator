...
import {
  ...
  InputOTPGroup,
  InputOTPSlot,
} from "@metanicator/ui/components/ui/input-otp";
...
import type { User } from "@metanicator/core/types";
    ...
    api
      ...
      .catch(() => {
        ...
        api
          ...
          .catch(() => {
            api.setToken(null);
            localStorage.removeItem("metanicator_token");
          });
      });
  }, [cliCallback]);
    ...
    async (value: string) => {
      ...
      try {
        if (cliCallback) {
          ...
          localStorage.setItem("metanicator_token", token);
          api.setToken(token);
          onTokenObtained?.();
  ...
  const handleCliAuthorize = async () => {
    ...
    try {
      ...
      if (authSourceRef.current === "localStorage") {
        ...
        const stored = localStorage.getItem("metanicator_token");
