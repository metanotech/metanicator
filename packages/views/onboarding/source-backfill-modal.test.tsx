...
vi.mock("@metanicator/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@metanicator/core/onboarding")>(
...
vi.mock("@metanicator/core/api", async () => {
  const actual =
    await vi.importActual<typeof import("@metanicator/core/api")>(
      "@metanicator/core/api",
    );
  return {
    ...actual,
    api: {
...
describe("SourceBackfillModal", () => {
  it("does not render when there is no user", () => {
    renderModal();
    expect(
      screen.queryByText(/How did you hear about Metanicator/i),
  ...
  it("opens for an onboarded user with empty source once agents completed enough issues", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about Metanicator/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("does not open once the per-user dismiss cap is reached on this browser", () => {
    window.localStorage.setItem("metanicator.source_backfill.dismiss.u1", "3");
    setUser({
      id: "u1",
      onboarded_at: "2026-01-01T00:00:00Z",
      onboarding_questionnaire: { source: [] },
