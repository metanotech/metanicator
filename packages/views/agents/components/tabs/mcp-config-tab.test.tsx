...
vi.mock("@metanicator/core/runtimes", async () => {
  const actual =
    await vi.importActual<typeof import("@metanicator/core/runtimes")>(
      "@metanicator/core/runtimes",
    );
  return {
    ...actual,
...
describe("McpConfigTab", () => {
  ...
  it("projects historical aggregate config into individually managed rows", () => {
    ...
    expect(screen.getByRole("heading", { name: /managed by metanicator/i })).toBeInTheDocument();
