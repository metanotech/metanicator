...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...
    ios: {
      ...
      // Metanicator Apple Developer team (and external users self-building a
      ...
      bundleIdentifier: isProd
        ? (process.env.EXPO_BUNDLE_IDENTIFIER_PROD ?? "ai.metanicator.mobile")
        : isStaging
          ? "ai.metanicator.mobile.staging"
          : (process.env.EXPO_BUNDLE_IDENTIFIER_DEV ?? "ai.metanicator.mobile.dev"),
    },
    plugins: [
      "expo-router",
      "expo-secure-store",
      ...
      [
        "expo-image-picker",
        {
          ...
          photosPermission:
            "Allow Metanicator to access your photos to attach images to issues and comments.",
