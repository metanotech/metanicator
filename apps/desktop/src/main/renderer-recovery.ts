...
export function createElectronReloadPrompt(
  ...
  return async (payload: ReloadPromptPayload): Promise<ReloadPromptResult> => {
    const result = await showMessageBox({
      ...
      defaultId: 0,
      cancelId: 1,
      title: "Metanicator needs to reload",
...
function rendererRecoveryDetail(payload: ReloadPromptPayload) {
  const guidance = [
    "Click Reload to refresh this window and keep using Metanicator.",
  ...
  if (payload.kind === "unresponsive") {
    guidance.push(
      "For macOS reports, an Activity Monitor sample of the Metanicator Helper (Renderer) process helps us find what blocked the app.",
    );
  }
