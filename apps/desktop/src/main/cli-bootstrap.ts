...
// Desktop prefers the bundled `metanicator` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/metanotech/metanicator/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "metanicator.exe" : "metanicator";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `metanicator-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `metanicator` binary. If one is already present at
