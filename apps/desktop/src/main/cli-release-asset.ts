const RELEASE_ARCHIVE_PREFIX = "metanicator-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `metanicator-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `metanicator_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `metanicator_${os}_${mappedArch}.${ext}`;
