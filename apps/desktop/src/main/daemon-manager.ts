...
const PREFS_PATH = join(homedir(), ".metanicator", "desktop_prefs.json");
...
// `metanicator daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/metanicator/cmd_daemon.go) to
 ...
 * a stuck start is an auth problem. Hits the same endpoint `metanicator auth status`
 ...
 * the user's default CLI profile at `~/.metanicator/` — neither its files nor its
 * health port.
 */
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["metanicator.exe"] : ["metanicator"];
 ...
 *   to `apps/desktop/resources/bin/metanicator`. `bundle-cli.mjs` populates this
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "metanicator.exe" : "metanicator";
...
/**
 * Returns a usable `metanicator` binary path. Priority:
 ...
 *   5. `metanicator` on PATH (dev convenience / user-installed via brew).
...
async function savePrefs(prefs: DaemonPrefs): Promise<void> {
  const dir = join(homedir(), ".metanicator");
...
async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "metanicator CLI is not installed" };
