...
// Keep the Go impl in sync: server/cmd/metanicator/cmd_daemon.go healthPortForProfile.
export const DEFAULT_HEALTH_PORT = 19514;

/**
 * Desktop owns only `~/.metanicator/profiles/desktop-<host>/`. The default profile
 * at `~/.metanicator/` — config, daemon log, and health port 19514 — belongs to the
...
//   ~/.metanicator/profiles/desktop-<host>/
...
export function profileDir(profile: string): string {
  assertResolvedProfile(profile);
  return join(homedir(), ".metanicator", "profiles", profile);
}
 ...
 * user's default profile at `~/.metanicator/config.json`.
 */
