#!/usr/bin/env node
// Builds the `metanicator` CLI from server/cmd/metanicator and copies the binary
...
//
// ldflags mirror `make build` so `metanicator --version` reports a meaningful
// version / commit / date.
//
...
function binaryNameForPlatform(platform) {
  return platform === "win32" ? "metanicator.exe" : "metanicator";
}
