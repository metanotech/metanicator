---
title: Using the CLI
description: Install and log in to the Metanicator CLI, and manage workspaces, issues, agents, and runtimes from the terminal.
---

import { Callout } from "fumadocs-ui/components/callout";

The Metanicator CLI both connects local runtimes and manages workspaces, issues, agents, and automations from the terminal. This page covers the common paths; the flags your installed version supports are always defined by each command's own `--help`.
...
If you only need to sign in again without overwriting other configuration, run `metanicator login`. On a machine without a browser, create a personal access token in the web settings first, then enter it with:

```bash
metanicator login --token
```
...
Subsequent commands use this workspace. A single command can override it with `--workspace-id`, or you can set `METANICATOR_WORKSPACE_ID`.

Invite members:

```bash
metanicator workspace member invite teammate@example.com
metanicator workspace member invite admin@example.com --role admin
```
...
metanicator issue get MUL-123
metanicator issue search "login failure"

# Create and update
metanicator issue create --title "Fix login failure"
metanicator issue status MUL-123 in_progress
metanicator issue assign MUL-123 --to "Backend Agent"

# Comments and runs
metanicator issue comment list MUL-123
metanicator issue comment add MUL-123 --content "Check the regression tests first"
metanicator issue runs MUL-123
```

Read long descriptions or comments from stdin to avoid dealing with newlines and quoting:

```bash
metanicator issue create --title "Upgrade notes" --description-stdin < notes.md
metanicator issue comment add MUL-123 --content-stdin < review.md
```

View the messages of a single run, or stop a task:

```bash
metanicator issue run-messages <task-id> --issue MUL-123
metanicator issue cancel-task <task-id> --issue MUL-123
```
...
metanicator agent get <agent-id>
metanicator agent create --help
metanicator agent update <agent-id> --help

metanicator skill list
metanicator skill get <skill-id>
metanicator skill import --url <skill-url>
metanicator agent skills add <agent-id> --skill-ids <skill-id>
```

When an import hits a skill with the same name, the default is to stop without modifying existing content. Pick by intent:

```bash
metanicator skill import --url <skill-url> --on-conflict overwrite
metanicator skill import --url <skill-url> --on-conflict rename
metanicator skill import --url <skill-url> --on-conflict skip
```
...
metanicator daemon logs --follow

metanicator runtime list
metanicator runtime rename <runtime-id> "Office Mac"
metanicator runtime usage <runtime-id>
metanicator runtime activity <runtime-id>
```

Deleting a runtime that still has active agents bound to it is refused by default. `metanicator runtime delete <id> --cascade` unbinds those agents, preserves their configuration and history, and cancels their active tasks.
...
`metanicator chat` reads **the external chat session an agent is currently handling**; it is mainly for agents in chat integrations, not a general command for browsing arbitrary workspace conversations.
...
Scripts should use JSON output rather than parsing the terminal-oriented tables. The output formats and pagination flags each command supports are defined by `metanicator <command> --help`.

## Profiles and configuration

The default configuration lives in `~/.metanicator/config.json`. Use `--profile <name>` to isolate a separate set of server address, token, default workspace, and daemon state:

```bash
metanicator setup self-host --profile staging \
  --server-url https://api.staging.example.com \
  --app-url https://app.staging.example.com

metanicator issue list --profile staging
```

Named profiles live in `~/.metanicator/profiles/<name>/config.json`. Inspect the current values:

```bash
metanicator config show
metanicator config show --profile staging
```

<Callout type="warning">
CLI configuration files contain tokens that can access Metanicator as you. Don't commit them to a repository, upload them to logs, or share them with others.
</Callout>

## Command reference

The tables below cover every current top-level command, grouped the way the CLI itself groups them. All commands accept the global flags `--server-url`, `--workspace-id`, `--profile`, and `--debug`. `--output` is registered per command, and the default format varies: `list` commands default to a table, while `get` and `create` commands mostly default to JSON. Full flags are defined by `metanicator <command> --help`.
...
| Command | Subcommand | Purpose | Key flags |
| --- | --- | --- | --- |
| `daemon` | `start` | Start the local daemon | `--foreground`, `--device-name`, `--runtime-name`, `--poll-interval`, `--heartbeat-interval`, `--agent-timeout` (`0` means no limit), `--max-concurrent-tasks`, `--no-auto-update`; each has a matching `METANICATOR_*` environment variable |
