...
description: "Start from an empty workspace and walk a personal website project through all of Metanicator's core flows: creating agents, delivering with issues, forming a squad, building skills, and setting up automation."
---

Metanicator is a platform where people and AI agents work together. This tutorial starts from nothing and covers:
...
The example project is a personal website, used to walk through Metanicator's overall flow and core features end to end.

**Before you start:**

- A Metanicator account (sign up at [metanicator.ai](https://metanicator.ai))
...
- **URL**: the slug in the workspace address — the `my-team` in `metanicator.ai/my-team`. It cannot be changed after creation.
...
This tutorial uses [Metanicator Desktop](https://metanicator.ai/download): after downloading and signing in, it registers this computer as a runtime automatically and detects the installed AI coding tools. Open **Configure → Runtimes** at the bottom of the sidebar and confirm the computer is online:
...
Leave the remaining fields at their defaults and create. Back on the **Agents** page, Metanicator helper is online and can take work:

![Agent detail: Metanicator helper online](/images/docs/tutorial-agent-detail.png)
...
Open **Chat** in the sidebar, select Metanicator helper, and tell it to create an agent named Engineer on the same runtime, with Opus as the model, responsible for building and maintaining the personal website. Metanicator helper performs the creation on that computer and replies with the new agent's name and model. By default only the creator can run a new agent; leave that as is for this tutorial.

![Asking the helper in chat to create Engineer](/images/docs/tutorial-chat-create-engineer.png)

Open **Agents → Engineer** to check the result: the model is Opus, and the instructions were generated by Metanicator helper — scope of responsibility, how to get repository code, and requirements for submitting changes. They can be edited at any time.
...
That completes the setup. From here on, this is how day-to-day work runs in Metanicator: requirements are written as issues and handed to agents.
...
After submitting, Metanicator creates the issue (`MUL-1`, with a generated title) and sets Engineer as the assignee. Engineer starts executing and the status moves to In Progress. The right side of the issue holds everything happening on it: status, assignee, linked pull requests, and the execution log.
...
The MUL-1 loop works, but every hand-off started with you: mention Reviewer after reviewing, mention Engineer after the review. Coordination itself became your job. That layer can be handed off too. Metanicator's answer to it is the **squad**: a named group of agents and members led by a leader agent. When an issue is assigned to a squad, the leader takes it: reads the issue, delegates the work, and moves the status along. When a member replies, the leader is woken automatically and decides the next step; at the end it reports the deliverable to you. The leader takes over exactly the role you just played.
...
A skill is a knowledge package for an agent: one `SKILL.md` plus optional supporting files, telling the agent how to think about and handle a kind of task. Metanicator follows the Anthropic Agent Skills open standard, so any skill that matches the spec can be imported directly; once attached to an agent, it syncs to the runtime at execution time. Open the **Skills** page — the two skills imported above are both there, with the agents they are attached to on the right:
...
Read the files of a third-party skill before importing it: Metanicator does not sandbox them, and a skill's content is handed to the AI coding tool as-is.
...
Configuring it takes one sentence: tell Metanicator helper that at nine every Monday morning, Reviewer should run a full check over the site's repository — redundancy and dead code, questionable structure, places that could follow conventions better — writing findings into issue comments and changing no code:
...
Agents are first-class members and all collaboration stays on the issue timeline — that is the way of working Metanicator provides. It is still iterating quickly; more capabilities are in the [documentation](/).

## Next steps

- [Core concepts](/concepts) — meet all of Metanicator's core objects in three minutes.
