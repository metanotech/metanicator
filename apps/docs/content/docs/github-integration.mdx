...
description: Link pull requests to Metanicator issues and watch development progress from the issue.
---

import { Callout } from "fumadocs-ui/components/callout";

Once GitHub is connected, Metanicator automatically links pull requests based on issue identifiers. Issue detail shows PR state, change size, CI results, and merge conflicts in place.
...
Self-hosted Metanicator can also connect self-hosted Forgejo, Gitea, or GitLab instances in parallel, with the same PR auto-linking, merge-to-Done, and CI display. The entry point is **Settings → Integrations → Git hosting**; see [Self-hosted Git hosting](/vcs-integration). Metanicator Cloud does not offer this entry point.
...
5. Return to Metanicator after the installation completes.
...
<Callout type="info">
The **GitHub connection** decides which repositories Metanicator receives PR events from; the **code repositories** setting decides which repositories agents can pick when running tasks. They serve different purposes and are configured separately.
</Callout>
...
| Co-authored-by | Adds `Co-authored-by: metanicator-agent <github@metanotech.id>` to commits created by agents. |
| Auto-link PRs | Detects issue identifiers in a PR's branch name, title, and body. |
| **PR card → CI & mergeability** | For each linked PR, Metanicator fetches an authenticated GitHub API snapshot and mirrors its **CI status** and **mergeability** onto the card (see **What the PR card shows** below). |
...
Metanicator ignores case and only matches the current workspace's issue prefix. One PR can link to multiple issues.
...
CI status and mergeability come from snapshots Metanicator pulls from the GitHub API, and the two are independent; merged or closed PRs no longer show either. When GitHub is temporarily unavailable, the card keeps the last snapshot and marks it stale instead of going blank.
...
A merged PR doesn't necessarily mean the issue is finished. Metanicator only moves an issue to `Done` when all of the following hold:
...
The same GitHub App installation can be connected to multiple Metanicator workspaces. GitHub events flow into each workspace separately and are matched against each workspace's own issue prefix.
...
Clicking **Disconnect** in **Settings → GitHub** only removes the relationship between the current Metanicator workspace and the installation; it does not uninstall the App from GitHub for you. Existing PR records are kept, and new events stop flowing into that workspace.

To revoke repository authorization on the GitHub side, uninstall the App or adjust its repository scope from your personal or organization GitHub App installations page. After uninstalling, every Metanicator workspace linked to that installation stops receiving events.

## Self-hosting setup

Metanicator Cloud doesn't need this section. Self-hosting requires creating your own GitHub App first.
...
| Field | Value |
|---|---|
| Homepage URL | Your Metanicator frontend address, e.g. `https://metanicator.example.com` |
...
If you don't need CI shown in Metanicator, you can skip the Checks and Commit statuses permissions and their events.
...
Take the slug from the App's public URL. For example, the slug of `https://github.com/apps/metanicator-acme` is `metanicator-acme`.

```dotenv
GITHUB_APP_SLUG=metanicator-acme
...
The next two variables are required for the PR card to show CI status and mergeability — Metanicator uses them to authenticate as the App and pull snapshots:
