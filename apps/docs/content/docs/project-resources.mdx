...
When an agent works an issue inside the project, Metanicator adds the project name, project description, and resource list to the run's context, and writes `.metanicator/project/resources.json` into the working directory.
...
Repositories are not limited to GitHub: any Git URL the runtime can reach works as a repository resource. Self-hosted Metanicator can also connect self-hosted Forgejo, Gitea, or GitLab under **Settings → Integrations → Git hosting**; see [Self-hosted Git hosting](/vcs-integration).
...
<Callout type="warning">
A local directory is not an isolated environment. The agent directly sees and modifies your current branch and uncommitted files; Metanicator does not automatically switch branches, stash, commit, push, or open PRs.
</Callout>
...
Beyond the code changes the agent makes, the runtime may also write into the directory the instruction files the current AI coding tool needs, plus `.metanicator/project/resources.json`. Add them to `.gitignore` if you do not want them under version control.

Metanicator never deletes a linked local directory when cleaning up run environments. The agent's changes to the directory are the same kind of changes as running an AI coding tool in your terminal yourself, and need the same review.

## Manage resources with the CLI

```bash
# Link a repository when creating a project
metanicator project create \
  --title "Agent UX" \
  --repo https://github.com/metanotech/metanicator

# List and add resources
metanicator project resource list <project-id>
metanicator project resource add <project-id> \
  --type github_repo \
...
# Remove a resource
metanicator project resource remove <project-id> <resource-id>
```
