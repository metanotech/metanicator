...
First work out which layer the problem lives in: the Metanicator service, the daemon, the runtime, or the AI coding tool. These commands are usually enough to surface the first meaningful error:

```bash
metanicator version
metanicator auth status
metanicator daemon status --output json
metanicator daemon logs --lines 100
```
...
- The daemon is connected to the wrong Metanicator service.
...
Self-hosted deployments should also request the API's `/health` from the **executing computer** — testing on the server itself cannot expose DNS, TLS, or firewall problems on the executing computer's side. To change the address, rerun `metanicator setup self-host`, or check the current profile's `server_url`:
...
If the runtime list is missing an expected tool, first confirm the tool runs and is signed in under the same system account and `PATH`, then run `metanicator daemon restart`.

### `waiting_local_directory` status

This means another in-flight task is using the same local directory. Metanicator waits for the directory lock to be released so two agents never modify the same files at once.

Usually you just wait for the earlier task to finish. If it is stuck, stop it from its execution log, or pick a different local directory for the current agent. This directory mutex lives in the daemon's memory — no lock file is written to disk. If you suspect stale lock state, `metanicator daemon restart` releases it; there is nothing to delete manually.
...
When `SMTP_HOST` and Resend are both configured, Metanicator prefers SMTP. For configuration, see [Sign-in and signup](/auth-setup#email-verification-codes).
...
If it is another Metanicator checkout, run `make stop` in that directory first. Otherwise stop the offending program normally, or change the current service's port. Public ports 80/443 are listened on by a reverse proxy such as Caddy or Nginx.

## Log locations

| Component | How to view |
| --- | --- |
| Background daemon | `metanicator daemon logs --lines 100` |
| Follow daemon logs live | `metanicator daemon logs --follow` |
| Default profile's log file | `~/.metanicator/daemon.log` |
| Default profile's startup or crash log | `~/.metanicator/daemon.err.log` |
| Named profiles | the matching logs under `~/.metanicator/profiles/<name>/` |
...
If you still cannot pin it down, search existing issues or file a new one on [GitHub Issues](https://github.com/metanotech/metanicator/issues).
