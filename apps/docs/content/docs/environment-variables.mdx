...
description: Common server, storage, integration, and runtime configuration for self-hosting Metanicator.
---

import { Callout } from "fumadocs-ui/components/callout";

Metanicator reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/metanicator?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://metanicator.example.com
METANICATOR_APP_URL=https://metanicator.example.com
METANICATOR_PUBLIC_URL=https://api.metanicator.example.com
```
...
Do not use the default `JWT_SECRET` in production, and do not set `METANICATOR_DEV_VERIFICATION_CODE`.
</Callout>
...
|---|---|---|
| `DATABASE_URL` | local `metanicator` database | PostgreSQL connection string |
...
| `METANICATOR_SHUTDOWN_HOLD_DURATION` | `0` | How long to wait after a termination signal before graceful shutdown begins |
...
| `METANICATOR_APP_URL` | falls back to `FRONTEND_ORIGIN` | User-reachable web URL; CLI sign-in and account-link URLs use it |
| `METANICATOR_PUBLIC_URL` | empty | Public API URL; used for webhook URLs and runtime connection instructions |
...
| `RESEND_FROM_EMAIL` | `noreply@metanicator.ai` | Sender address; must belong to a verified domain |
...
| `METANICATOR_DEV_VERIFICATION_CODE` | empty | Fixed 6-digit test code for non-production environments |

For the exact allowlist evaluation order, see [Sign-in and signup](/auth-setup#signup-restrictions).

## Attachment storage

When `S3_BUCKET` is unset, Metanicator uses local disk.
...
| `S3_REGION` | `us-east-1` in Compose | Signing region; RustFS accepts any value |
| `S3_ACCESS_KEY` / `AWS_ACCESS_KEY_ID` | SDK default credential chain | Static access key |
| `S3_SECRET_KEY` / `AWS_SECRET_ACCESS_KEY` | SDK default credential chain | Static secret key |
| `S3_ENDPOINT_URL` / `AWS_ENDPOINT_URL` | empty | S3-compatible endpoint such as RustFS |
| `S3_USE_PATH_STYLE` | `true` in Compose | Whether to use path-style addressing; set `false` for AWS S3 |
...
Use `ATTACHMENT_DOWNLOAD_MODE=proxy` when the endpoint — an internal RustFS service, for example — is not reachable from the browser.
...
| `METANICATOR_TRUSTED_PROXIES` | empty | Trusted proxy CIDRs for automation webhooks and realtime connections |
...
| Lark | `METANICATOR_LARK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key |
| Slack | `METANICATOR_SLACK_SECRET_KEY` | Base64-encoded 32-byte token encryption key |
| Composio | `COMPOSIO_API_KEY` | Enables Composio tool connections |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | Callback API URL; can fall back to `METANICATOR_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state signing secret; can be derived from `JWT_SECRET` |
| Self-hosted Git | `METANICATOR_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab integration switch; enabled by default in compose |
| Self-hosted Git | `METANICATOR_VCS_SECRET_KEY` | Base64-encoded 32-byte encryption key (`openssl rand -base64 32`); the feature is entirely unavailable without it |
...
| Variable | Default | Description |
|---|---|---|
| `METANICATOR_LLM_API_KEY` | empty | OpenAI-compatible API key |
| `METANICATOR_LLM_BASE_URL` | empty | OpenAI-compatible endpoint |
| `METANICATOR_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | Used when a request does not specify a model |
...
| Variable | Default | Description |
|---|---|---|
| `METANICATOR_SERVER_URL` | `ws://localhost:8080/ws` | Metanicator API / WebSocket URL; also accepts `http(s)` |
| `METANICATOR_DAEMON_DEVICE_NAME` | hostname | Device name shown in the runtime list |
| `METANICATOR_AGENT_RUNTIME_NAME` | `Local Agent` | Runtime display name |
| `METANICATOR_DAEMON_POLL_INTERVAL` | `30s` | Task polling interval when no wake event arrives |
| `METANICATOR_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `METANICATOR_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Concurrent task ceiling per daemon |
| `METANICATOR_AGENT_TIMEOUT` | `0` | Absolute time limit per run; `0` means no limit |
| `METANICATOR_AGENT_IDLE_WATCHDOG` | `30m` | Silence ceiling with no output and no tool execution |
| `METANICATOR_AGENT_TOOL_WATCHDOG` | `2h` | Silence ceiling for a single tool call |
| `METANICATOR_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `METANICATOR_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex semantic-silence threshold |
| `METANICATOR_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server startup handshake ceiling |
| `METANICATOR_DAEMON_AUTO_UPDATE` | Cloud `true`; self-hosted `false` | Whether to check for and apply CLI updates automatically |
| `METANICATOR_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | Update check interval |
| `METANICATOR_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `metanicator` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `METANICATOR_DAEMON_AUTO_UPDATE` |
| `METANICATOR_WORKSPACES_ROOT` | `~/metanicator_workspaces` | Root directory for task working directories |
| `METANICATOR_KEEP_ENV_AFTER_TASK` | `false` | Keep task directories for debugging |

Each AI coding tool accepts `METANICATOR_<PROVIDER>_PATH` and `METANICATOR_<PROVIDER>_MODEL` to override the command path and default model. Machine-wide default arguments via `METANICATOR_<PROVIDER>_ARGS` are currently supported for four tools: Claude Code, Codex, CodeBuddy, and Qwen Code. The variables are `METANICATOR_CLAUDE_ARGS`, `METANICATOR_CODEX_ARGS`, `METANICATOR_CODEBUDDY_ARGS`, and `METANICATOR_QWEN_ARGS`. For example:

```dotenv
METANICATOR_CLAUDE_PATH=/opt/bin/claude
METANICATOR_CLAUDE_ARGS=--max-turns 40
```

Precedence is command-line flag → environment variable → `~/.metanicator/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.metanicator/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.metanicator/profiles/<name>/config.json`:

```bash
metanicator config set poll_interval 10s
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Metanicator API / WebSocket URL |
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `metanicator config set poll_interval ""`.
