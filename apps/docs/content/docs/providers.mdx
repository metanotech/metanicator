...
description: Compare the AI coding tools Metanicator supports, and how models, MCP, sessions, and skills differ.
---

import { Callout } from "fumadocs-ui/components/callout";

Metanicator does not replace the AI coding tool you use. A runtime invokes the tool already installed and signed in on the computer, then writes progress and results back to the issue.
...
- whether Metanicator needs to configure MCP for it;
...
| Tool | Detected command | Session resumption | Metanicator-managed MCP | Skill injection path |
|---|---|---:|---:|---|
...
"Metanicator-managed MCP" means you can define MCP servers in the agent configuration and Metanicator passes them to the tool before a run. It does not mean the tool has no other way to configure MCP.
...
If you do not pick a model, Metanicator uses the tool's own default. If the list is empty, confirm the runtime is online and the tool is signed in, then refresh the model list.
...
Metanicator stores the session identifier the tool returns. The next time work continues on the same issue, the runtime tries to resume that session; when it cannot, it creates a new session and keeps going. Pi stores a local session file path, so it depends more on the original computer and file.
...
Before a run starts, Metanicator writes the skills bound to the agent into the paths in the table above, so each tool discovers them through its own native mechanism.

Skills that already exist in the repository are never overwritten. On a name collision, Metanicator uses a different directory name for the injected copy; after the run, it cleans up only the files it created.
