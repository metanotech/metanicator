...
Metanicator creates a [task](/tasks) for the agent. Once an online runtime claims the task, it invokes the AI coding tool on that machine; progress, comments, and results then appear in the issue.
...
When the assignee is a [squad](/squads), the same expectations apply: Metanicator enqueues the leader, who moves the parent issue to `in_progress` on the first turn and keeps it there while members work. The leader only moves it to `in_review` once the overall goal is met — a successful first dispatch is not completion.
...
```bash
metanicator issue assign MUL-42 --to "Agent name"
metanicator issue assign MUL-42 --unassign
```

In scripts, use `--to-id <uuid>` to avoid matching the wrong object with the same name. Look up UUIDs with `metanicator agent list --output json` or `metanicator workspace member list --output json`.
