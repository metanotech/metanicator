...
写 Metanicator 代码、改翻译、写中文产品文案，都从这一页查。
...
| `packages/ui` | 无业务依赖 | `@metanicator/core`、业务逻辑 |
...
| `apps/mobile/` | `@metanicator/core` 的类型和纯函数 | Web/Desktop 的 React 页面、store 和平台实现 |
...
### 核心区分：日常词 vs Metanicator 专有词

Metanicator 的产品名词分两类：

- **日常词** —— 用户会直接说出口的词。**完整翻译成中文**，是不是数据库实体都一样：`issue` → 任务、`workspace` → 工作区、`project` → 项目。
- **Metanicator 专有词** —— 中文里没有对应词的概念（`skill`），或用户可能要照着敲、照着比对的 schema 标识符（`todo`、`in_progress`、`task_id`）。**用小写英文**呈现，读起来像类型名。
...
- **代码引用和字面命令**保持英文：`metanicator issue ...`、Slack 与飞书的 `/issue` 斜杠命令。
- **`skill`** 在中文里仍用小写英文 —— Metanicator 特有概念，没有公认中文译法；标题可大写为 `Skills`。
- **表示「问题」的 issue** 不是这个实体：机器卡片上的 `{{count}} issues` 是 `{{count}} 个异常`，不是 `任务`。

**为什么 `issue` 要译，而 `skill` 不译**：用户每天都在提任务、看任务，而 "issue" 这个词在中文、日文、韩文里除了开发圈黑话之外没有含义，当地人本来就用「任务」在说这件事。`skill` 则是 Metanicator 特有概念，没有哪个中文词能承载它的意思。
...
| 类别 | 词 |
| --- | --- |
| 品牌 | **Metanicator**、GitHub、Slack、Google、Anthropic、OpenAI、Claude、Codex、Cursor、Linear、Jira |
