...
description: How Metanicator connects computers, discovers AI coding tools, and executes tasks.
---

import { Callout } from "fumadocs-ui/components/callout";

Metanicator records and coordinates the work; connected computers execute it. The **daemon** on a computer claims tasks and invokes the AI coding tools installed on that machine.

## Daemon vs runtime

- The **daemon** is the Metanicator background process running on one computer. It connects to the server, discovers local tools, claims tasks, and reports results back.
...
The AI coding tools a local runtime invokes, those tools' own login credentials, and your local code directories all stay on the connected computer. The Metanicator server does not execute commands on behalf of local tools, and it does not automatically upload your entire working directory.
...
With Metanicator Desktop, the app starts the daemon automatically — no extra command needed.

On the web, a remote computer, or a headless environment, [install the Metanicator CLI](/cli) first, then run:
...
| Command | Purpose |
| --- | --- |
| `metanicator daemon status` | Show daemon and connection status |
| `metanicator daemon logs -f` | Follow the logs |
| `metanicator daemon restart` | Restart the daemon and re-detect local tools |
| `metanicator daemon stop` | Stop the daemon |
| `metanicator daemon start --foreground` | Run in the current terminal, for debugging |
...
Once a limit is reached, new tasks keep queueing. You can adjust a single agent's concurrency in the agent settings, and the machine-wide cap through `METANICATOR_DAEMON_MAX_CONCURRENT_TASKS`. Parallel runs compete for machine capacity, tool account quota, and the same working directory all at once.
...
A custom profile does not add a new communication protocol. You still pick one of the protocol families Metanicator already supports (the tool's integration protocol type; see [AI coding tools comparison](/providers)), and the command itself must be compatible with that family.
...
```bash
metanicator runtime profile set-path <profile-id> --path /absolute/path/to/command
```

Remove the path override:

```bash
metanicator runtime profile unset-path <profile-id>
```
...
1. Run `metanicator daemon status` to confirm the daemon is running.
2. Run `metanicator daemon logs -f` to look for login, network, or tool-detection errors.
3. Run `command -v <tool-command>` in the same environment to confirm the daemon can find it.
4. Open Metanicator's **Runtimes** page and check whether the target computer and the matching AI coding tool show as online.
5. After a tool install, a path change, or a profile update, run `metanicator daemon restart`.
