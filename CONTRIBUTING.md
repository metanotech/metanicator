...
This guide documents the local development workflow for contributors working on the Metanicator codebase.
...
By submitting a contribution to Metanicator — a pull request, a patch, or any
other work — you agree to condition 2 of the [Metanicator License](LICENSE):

- your contribution is submitted under the Metanicator License as a whole (the
...
- the producer can adjust the Metanicator License to be more strict or relaxed
  as deemed necessary.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=metanicator`
...
POSTGRES_PORT=5432
DATABASE_URL=postgres://metanicator:metanicator@localhost:5432/metanicator?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=metanicator_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://metanicator:metanicator@localhost:5432/metanicator_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../metanicator-feature -b feat/my-change main
cd ../metanicator-feature
make dev
```
...
- main checkout
  - database: `metanicator`
  - backend: `8080`
  - frontend: `3000`
- worktree checkout
  - database: `metanicator_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`metanicator login`).
...
whatever server is configured in `~/.metanicator/config.json`. That's fine for
...
Example: worktree at `../metanicator-feat-auth` produces profile
`dev-metanicator_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `METANICATOR_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.metanicator/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `metanicator` commands automatically use the same
...
rm -rf "$HOME/.metanicator/profiles/$PROFILE"
```
...
1. Compiles the `metanicator` CLI from `server/cmd/metanicator` into
   `apps/desktop/resources/bin/metanicator`
...
backend logs. If you set `METANICATOR_DEV_VERIFICATION_CODE=888888` before starting
...
- `DESKTOP_APP_SUFFIX` = `<folder>-<offset>` — its own single-instance lock /
  `userData`, and an app named `Metanicator Canary <folder>-<offset>` so it is
  distinguishable in Cmd+Tab. The offset keeps it unique across worktrees that
  share a folder name at different paths.

The primary checkout is left untouched (`5173`, `Metanicator Canary`). Set either
...
Nothing in this flow touches the system-installed `metanicator` or the default
`~/.metanicator/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.metanicator/config.json` | `~/.metanicator/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.metanicator/daemon.pid` | `~/.metanicator/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/metanicator_workspaces/` | `~/metanicator_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `metanicator_<slug>_<hash>` |
| Desktop profile | `desktop-api.metanicator.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U metanicator -d postgres -At -c "select datname from pg_database order by datname;"
```
