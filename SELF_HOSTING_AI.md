...
This document is designed for AI agents to execute. Follow these steps exactly to deploy a local Metanicator instance and connect to it.
...
Wait for the server output `✓ Metanicator server is running and CLI is ready!` before running `metanicator setup self-host`.

**Expected result:**
- Frontend at http://localhost:3000
- Backend at http://localhost:8080
- `metanicator` CLI installed and configured for localhost
...
The `metanicator setup self-host` command will:
...
2. Run `make selfhost`
3. Run `metanicator setup self-host --port <PORT> --frontend-port <FRONTEND_PORT>`
...
- **Daemon issues:** `metanicator daemon logs`
