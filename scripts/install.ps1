# Metanicator installer for Windows — one command to get started.
#
# Install CLI (default): connects to metanicator.ai
#   irm https://raw.githubusercontent.com/metanotech/metanicator/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Metanicator server + installs CLI + configures
#   $env:METANICATOR_MODE="local"; irm https://raw.githubusercontent.com/metanotech/metanicator/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/metanotech/metanicator.git"
$RepoWebUrl    = "https://github.com/metanotech/metanicator"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".metanicator\server"
$InstallDir    = if ($env:METANICATOR_INSTALL_DIR) { $env:METANICATOR_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/metanotech/metanicator/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:METANICATOR_SELFHOST_REF) {
        return $env:METANICATOR_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = metanicator version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Metanicator CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Metanicator requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/metanotech/metanicator/releases/download/$latest/metanicator-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "metanicator-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "metanicator.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/metanotech/metanicator/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "metanicator.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "metanicator-cli-$version-windows-$arch.zip"
        $legacyAsset = "metanicator_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "metanicator.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".metanicator\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "metanicator.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "metanicator.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "metanicator.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "metanicator.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Metanicator CLI installed to $binDir\metanicator.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "metanicator") {
        ...
        if ($isUpToDate) {
            Write-Ok "Metanicator CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Metanicator CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Metanicator CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "metanicator")) {
        Write-Fail "CLI installed but 'metanicator' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Metanicator self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:METANICATOR_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Metanicator server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Metanicator repository..."
    ...
    Write-Info "Pulling official Metanicator images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Metanicator services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Metanicator server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Metanicator - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Metanicator CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     metanicator setup               " -NoNewline; Write-Host "# Connect to Metanicator Cloud (metanicator.ai)" -ForegroundColor DarkGray
    Write-Host "     metanicator setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:METANICATOR_MODE="with-server"; irm https://raw.githubusercontent.com/metanotech/metanicator/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Metanicator - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Metanicator server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     metanicator setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:METANICATOR_MODE="stop"; irm https://raw.githubusercontent.com/metanotech/metanicator/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Metanicator services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Metanicator installation found at $InstallDir"
    }

    if (Test-CommandExists "metanicator") {
        try {
            metanicator daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:METANICATOR_MODE) { $env:METANICATOR_MODE.ToLower() } else { "default" }
