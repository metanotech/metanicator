...
Deploy Metanicator on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`metanicator` CLI** and runs the **agent daemon** on their own machine.
...
```powershell
# 1. Install CLI + provision the self-host server
$env:METANICATOR_MODE="with-server"; irm https://raw.githubusercontent.com/metanotech/metanicator/main/scripts/install.ps1 | iex
...
This installs the `metanicator` CLI, checks out the latest self-host assets, pulls the official Metanicator images from GHCR, and configures everything for localhost.
...
`make selfhost-build` uses local `metanicator-backend:dev` / `metanicator-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `METANICATOR_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `METANICATOR_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
If you already run a Kubernetes cluster, you can deploy Metanicator there instead of Docker Compose using the released OCI Helm chart at `oci://ghcr.io/metanicator-ai/charts/metanicator` or the source chart at [`deploy/helm/metanicator/`](deploy/helm/metanicator/). It targets a typical k3s / k8s setup with an Ingress controller and a default `ReadWriteOnce` StorageClass — authored against k3s + Traefik + `local-path`, and should work on any cluster with minor tweaks.

The chart creates the following resources in the target namespace:

- `metanicator-postgres` — `pgvector/pgvector:pg17` backed by a 10Gi PVC
- `metanicator-backend` — Go API/WS server. Backed by a 5Gi `ReadWriteOnce` uploads PVC by default; set `backend.uploads.persistence.enabled=false` when you have configured S3 (`backend.config.s3Bucket`) and don't want the chart to declare the PVC at all.
- `metanicator-frontend` — Next.js standalone server
- Two `Ingress` resources: one for the web host, one for the backend host
- `metanicator-config` ConfigMap (rendered from `values.yaml`)

The `metanicator-secrets` Secret is **not** managed by the chart — you create it once with `kubectl` so real values never need to land in git.

> **Runtime frontend upstreams:** current `metanicator-web` images read `REMOTE_API_URL` and `DOCS_URL` when the Next.js server runs, so API/docs upstream changes do not require a web rebuild. The chart defaults `REMOTE_API_URL` to this release's backend Service. `frontend.compatibility.backendAlias` exists only for legacy images that still baked `REMOTE_API_URL=http://backend:8080` at build time.
...
The chart defaults to `metanicator.dev.lan` (web) and `api.metanicator.dev.lan` (backend). Pick one of:

- **`/etc/hosts`** on every machine that needs access (developer laptops + the machine running the daemon):

  ```text
  192.168.1.206  metanicator.dev.lan api.metanicator.dev.lan
  ```
...
```bash
kubectl create namespace metanicator
```

### Step 3 — Create the `metanicator-secrets` Secret

The chart references this Secret by name. Create it once with random values:

```bash
kubectl -n metanicator create secret generic metanicator-secrets \
  --from-literal=JWT_SECRET="$(openssl rand -hex 32)" \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -hex 16)" \
  --from-literal=RESEND_API_KEY="" \
  --from-literal=GOOGLE_CLIENT_SECRET="" \
  --from-literal=CLOUDFRONT_PRIVATE_KEY="" \
  --from-literal=METANICATOR_DEV_VERIFICATION_CODE=""
```
...
```bash
helm install metanicator oci://ghcr.io/metanicator-ai/charts/metanicator \
...
```bash
helm show values oci://ghcr.io/metanicator-ai/charts/metanicator \
...
```bash
helm install metanicator deploy/helm/metanicator -n metanicator
```

Watch the pods come up:

```bash
kubectl -n metanicator get pods -w
```
...
```bash
curl -H "Host: api.metanicator.dev.lan" http://<ingress-ip>/healthz
# {"status":"ok","checks":{"db":"ok","migrations":"ok"}}
```

Then open http://metanicator.dev.lan in your browser.
...
- **Recommended (production):** patch the Secret with a real Resend key, then restart the backend:
  ...
  ```bash
  kubectl -n metanicator patch secret metanicator-secrets --type=merge \
    -p '{"stringData":{"RESEND_API_KEY":"re_xxx"}}'
  kubectl -n metanicator rollout restart deploy/metanicator-backend
  ```
...
- **Without email configured:** the verification code is generated server-side and printed to the backend pod logs (look for `[DEV] Verification code for ...:`). Useful for one-off testing.
  ...
  ```bash
  kubectl -n metanicator logs -f deploy/metanicator-backend | grep "Verification code"
  ```

- **Deterministic local/private testing:** set `backend.config.appEnv: development` in your values file and `METANICATOR_DEV_VERIFICATION_CODE=888888` in the Secret, then `helm upgrade` and restart. This fixed code is ignored when `APP_ENV=production`.

  ```bash
  helm upgrade metanicator oci://ghcr.io/metanicator-ai/charts/metanicator \
    --version <chart-version> \
    -n metanicator \
    -f my-values.yaml --set backend.config.appEnv=development
  kubectl -n metanicator patch secret metanicator-secrets --type=merge \
    -p '{"stringData":{"METANICATOR_DEV_VERIFICATION_CODE":"888888"}}'
  kubectl -n metanicator rollout restart deploy/metanicator-backend
  ```
...
```bash
metanicator setup self-host \
  --server-url http://api.metanicator.dev.lan \
  --app-url http://metanicator.dev.lan
```
...
```bash
kubectl -n metanicator rollout restart deploy/metanicator-backend deploy/metanicator-frontend
```

To upgrade to a specific Metanicator release, upgrade to the matching chart version. The released chart defaults its app images to the matching Git tag:
...
```bash
helm -n metanicator rollback metanicator
```

> **Upgrading from `v0.3.4` to `v0.3.5+` fails with `refusing to drop legacy daily rollups: ...`?** As of MUL-2957 the `migrate up` command runs an idempotent monthly-slice backfill automatically before applying migration `103`, so a clean upgrade is a single `helm upgrade` + backend rollout. If you are still on a pre-MUL-2957 binary or the auto-hook fails, run the standalone backfill against the same database the chart is using (`kubectl -n metanicator exec deploy/metanicator-backend -- ./backfill_task_usage_hourly --sleep-between-slices=2s`), then restart the backend deployment to re-apply migrations. See [Advanced Configuration → Usage Dashboard Rollup](SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup) for the full recovery flow.

### Tearing down

```bash
# Remove the workloads but keep the PVCs and the Secret
helm -n metanicator uninstall metanicator

# Wipe everything, including PostgreSQL data and uploads
kubectl delete namespace metanicator
```
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in Metanicator's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
## Switching to Metanicator Cloud

If you've been self-hosting and want to switch your CLI to [Metanicator Cloud](https://metanicator.ai):

```bash
metanicator setup
```

This reconfigures the CLI for metanicator.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `METANICATOR_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
...
If you prefer configuring the CLI step by step instead of `metanicator setup`:
