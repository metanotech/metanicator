...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	MetanicatorUserID  pgtype.UUID // the installer's Metanicator account
...
// identity is the supplied metanicatorUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Metanicator user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, metanicatorUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	isMember, err := qtx.IsWorkspaceMember(ctx, row.WorkspaceID, metanicatorUserID)
	if err != nil {
	...
	if err != nil {
		...
		// metanicator_user_id differs from ours, so the WHERE clause on
...
// (installation_id, lark_open_id) when metanicator_user_id matches (the
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *ChannelStore, p InstallerBindParams) error {
	...
	isMember, err := q.IsWorkspaceMember(ctx, p.WorkspaceID, p.MetanicatorUserID)
	if err != nil {
	...
	_, err = q.CreateLarkUserBinding(ctx, CreateUserBindingParams{
		...
		MetanicatorUserID:  p.MetanicatorUserID,
...
// open_id) pair and points at a different Metanicator user. Account
