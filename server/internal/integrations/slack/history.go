...
// failed) read so the unified `metanicator chat history` / `metanicator chat thread`
...
// `metanicator chat history` (channel overview) and `metanicator chat thread [id]`
...
// boundary for `metanicator chat thread <id>` (the agent supplies only a
...
// drills into with `metanicator chat thread <id>`. Backs `metanicator chat history`.
...
// threads) reads its linear conversation. Backs `metanicator chat thread [id]`.
...
// agent can drill in with `metanicator chat thread <id>`.
