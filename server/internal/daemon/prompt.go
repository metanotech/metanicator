...
func sessionContinuityNoticeFor(task Task) string {
	...
	// Web chat (no channel type) and every channel Metanicator cannot read back.
...
func buildPromptBody(task Task, provider string) string {
	...
	var b strings.Builder
	b.WriteString("You are running as a local coding agent for a Metanicator workspace.\n\n")
	...
	fmt.Fprintf(&b, "Start by running `metanicator issue get %s --output json` to understand your task, then complete it.\n", task.IssueID)
	fmt.Fprintf(&b, "For comment history, follow the rule in your runtime workflow file (assignment-triggered tasks treat the read as mandatory). Scan the threads first with `metanicator issue comment list %s --roots-only --summary --output json`, then expand only what matters with `--thread <thread-id> --tail 30`. For `--since` incremental polling, pagination, and folding, see `metanicator issue comment list --help`.\n", task.IssueID)
	return b.String()
}

// buildQuickCreatePrompt constructs a prompt for quick-create tasks. The
// user typed a single natural-language sentence in the create-issue modal;
// the agent's job is to translate it into one `metanicator issue create` CLI
...
// call `metanicator issue get` or attempt to comment — there's nothing to read
// or reply to.
func buildQuickCreatePrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a quick-create assistant for a Metanicator workspace.\n\n")
	b.WriteString("A user captured the following input via the quick-create modal. There is NO existing issue. Your job is to create a well-formed issue from this input with a single `metanicator issue create` command.\n\n")
	...
	b.WriteString("    - When the user names someone (\"assign to X\" / \"@X\"), call `metanicator workspace member list --output json`, `metanicator agent list --output json`, and `metanicator squad list --output json` and find the matching entity by display name. Squads are first-class assignees too — a squad name (e.g. \"Super Human\") routes work to the squad leader, who then delegates. On a clean unambiguous match, prefer `--assignee-id <uuid>` using the `user_id` (member) or `id` (agent or squad) from that JSON — UUID matching is exact and robust to name collisions in workspaces with overlapping names. `--assignee <name>` (fuzzy) is acceptable as a fallback when names are unambiguous. On no match or ambiguous match, do NOT pass either flag — instead append a final line to the description: `Unrecognized assignee: X`.\n")
	...
	b.WriteString("- Run exactly one `metanicator issue create --output json` invocation. Do not retry for any reason — even on non-zero exit. The issue may already exist; another attempt would create a duplicate.\n")
	...
	b.WriteString("- Do NOT call `metanicator issue get` or `metanicator issue comment add` — there is no issue to query or comment on.\n")
...
func buildCommentPrompt(task Task, provider string) string {
	...
	if task.TriggerCommentContent != "" {
		...
		if len(task.CoalescedComments) > 0 {
			...
			fmt.Fprintf(&b, "\nIf you need the surrounding discussion for any of them, fetch its thread with `metanicator issue comment list %s --thread <thread-id> --tail 30 --output json` using the thread id shown above.\n\n", task.IssueID)
			...
			if task.NewCommentsSince != "" {
				fmt.Fprintf(&b, "Start with `metanicator issue comment list %s --since %s --output json`. Treat that as a candidate window, not a guarantee — it also carries unrelated comments, and a retried run can carry ids older than the window. Check every id above against the result.\n\n",
					task.IssueID, task.NewCommentsSince)
			}
			fmt.Fprintf(&b, "Fetch each id you still need directly: `metanicator issue comment list %s --thread <comment-id> --tail 30 --output json`. `--thread` accepts a reply id, not just a thread root, so you do not need to know which thread the comment lives in. If it is older than those 30 replies, page back with the `Next reply cursor` values (`--before` / `--before-id`) until it appears. Do not finish this turn until every id above is accounted for.\n\n",
				task.IssueID)
		}
		if taskIsSquadLeader(task) {
			fmt.Fprintf(&b, "⚠️ **Squad leader no_action rule:** If you decide no action is needed, call `metanicator squad activity %s no_action --reason \"...\"` and EXIT. DO NOT post any comment — not even one that says \"no action needed\" or \"exiting silently\". The squad activity call records your decision; a comment is redundant noise.\n\n", task.IssueID)
		}
	}
	fmt.Fprintf(&b, "Start by running `metanicator issue get %s --output json` to understand your task, then decide how to proceed.\n\n", task.IssueID)
	...
	if hint := execenv.BuildNewCommentsHint(task.IssueID, task.TriggerCommentID, task.TriggerThreadID, task.NewCommentsSince, task.NewCommentCount); hint != "" {
		...
		fmt.Fprintf(&b, "Read the discussion: scan with `metanicator issue comment list %s --roots-only --summary --output json`, then expand what matters with `--thread <thread-id> --tail 30`.\n\n", task.IssueID)
	}
...
func buildChatPrompt(task Task) string {
	...
	if task.ChatIntro {
		var b strings.Builder
		b.WriteString("You are running as a chat assistant for a Metanicator workspace.\n")
	...
	// like "what did you just talk about" sends it to read Metanicator instead of the
	// channel conversation. A web-only chat session gets no such block — its
	// history is the Metanicator chat_session the agent already resumes.
	//
	// The history half is Slack-only, and that is a real server constraint, not a
	// simplification: `metanicator chat history` / `metanicator chat thread` are served by
	...
	if task.ChatChannelType != "" {
		...
		fmt.Fprintf(&b, "You are operating inside a %s conversation — not the Metanicator web app. This conversation and its history live in %s, NOT in Metanicator; never look in Metanicator issues or comments for it.\n", platform, platform)
		if task.ChatChannelType == execenv.ChannelTypeSlack {
			b.WriteString("The message below may be only what triggered you. Read the conversation with:\n")
			b.WriteString("- `metanicator chat history --output json` — the channel overview: recent top-level messages, each thread tagged with a `thread_id` and `reply_count`. It does NOT expand thread contents.\n")
			b.WriteString("- `metanicator chat thread [<thread_id>] --output json` — read one thread's messages; omit the id to read the thread you are in, or pass a `thread_id` from the overview to read a specific thread.\n")
			if task.ChatInThread {
				b.WriteString("You were @mentioned inside a thread: start with `metanicator chat thread` to read it; if you need the wider channel, run `metanicator chat history` and open a specific thread with `metanicator chat thread <thread_id>`.\n")
			} else {
				b.WriteString("You were @mentioned at the channel top level: start with `metanicator chat history` to see the channel, then read a specific thread's contents with `metanicator chat thread <thread_id>`.\n")
			}
			...
			fmt.Fprintf(&b, "Work from the context already provided to you below — Metanicator has no history reader for %s, so there is no command that can fetch more of this conversation. If you genuinely need earlier context that is not here, ask the user for it rather than guessing.\n", platform)
		}
	...
	// have expired. `metanicator attachment download <id>` re-signs at click
	...
	if len(task.ChatMessageAttachments) > 0 {
		...
		b.WriteString("Use `metanicator attachment download <id>` to fetch each file locally before referring to it.\n")
		b.WriteString("When creating an issue that should preserve one of these attachments, pass `--attachment-id <id>` to `metanicator issue create` in addition to keeping the attachment markdown inline.\n")
	}
	...
	// that platform, not the Metanicator chat UI, so this binding does not apply.
	...
	if task.ChatChannelType == "" {
		b.WriteString("\nTo include a file or image you produced in your reply, run `metanicator attachment upload <local-path>`. The file binds to your reply automatically and appears as an attachment card below it even if you paste nothing. The command also returns a `markdown` snippet you may paste on its own line to place the item where you want it (files render as a card, images inline).\n")
	} else {
		fmt.Fprintf(&b, "\nThis reply is delivered to %s as text. You cannot attach a file to it: `metanicator attachment upload` binds to a Metanicator chat reply, which this is not. If you produce a file, describe it in words — never write its local path as a link, and never upload it and then write as though it arrived.\n", channelDisplayName(task.ChatChannelType))
	}
	return b.String()
}
...
func buildAutopilotPrompt(task Task) string {
	...
	if task.AutopilotID != "" {
		fmt.Fprintf(&b, "Start by running `metanicator autopilot get %s --output json` if you need the full autopilot configuration, then complete the instructions above.\n", task.AutopilotID)
	} else {
...
// promoted to leader and handed the leader rules (mandatory `metanicator squad
