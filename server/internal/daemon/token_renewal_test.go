...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "metanicator login") {
		t.Fatalf("401 warning must tell the user to run 'metanicator login', got: %s", out)
	}
}
...
// and the operator would never see a "run metanicator login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "metanicator login") {
		t.Fatalf("expected the actionable 'run metanicator login' hint in the WARN, got: %s", out)
	}
}
