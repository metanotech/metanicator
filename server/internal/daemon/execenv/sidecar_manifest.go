...
const sidecarManifestFile = ".metanicator_sidecar_manifest.json"
...
//     so the agent still discovers the Metanicator skill, just under a
//     different directory name.
//   - For Metanicator-only namespaces (.agent_context/issue_context.md,
//     .metanicator/project/resources.json) the caller swallows the error
...
// down a Metanicator skill without colliding with a user-installed skill
...
// collision we append `-metanicator`, then `-metanicator-2`, `-metanicator-3`,
...
// baseSlug: the bare slug first, then `-metanicator`, then numbered variants.
//
...
func skillSlugCandidate(baseSlug string, attempt int) string {
	...
	case attempt <= 0:
		return baseSlug
	case attempt == 1:
		return baseSlug + "-metanicator"
	default:
		return fmt.Sprintf("%s-metanicator-%d", baseSlug, attempt)
	}
}
...
// (.agent_context/, .metanicator/, .claude/skills/, .github/skills/,
...
// issue-review-metanicator. A managed skill directory is platform-owned — the
