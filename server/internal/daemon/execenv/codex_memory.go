...
// — users cannot audit or edit the contents from any Metanicator UI.
//
// This conflicts with the Metanicator daemon's context model. Metanicator
...
//     unrelated host-project context into Metanicator tasks. The reproduction
//     in github.com/metanotech/metanicator#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Metanicator issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Metanicator task
// (and accept the leak risk) can keep the feature enabled by setting
// `METANICATOR_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Metanicator-owned, user-visible,
...
// MetanicatorCodexMemoryEnv is the env var users can set to keep Codex
...
const MetanicatorCodexMemoryEnv = "METANICATOR_CODEX_MEMORY"

const (
	metanicatorMemoryFeatureBeginMarker = "# BEGIN metanicator-managed memory-feature (do not edit; regenerated by daemon)"
	metanicatorMemoryFeatureEndMarker   = "# END metanicator-managed memory-feature"
	metanicatorMemoryConfigBeginMarker  = "# BEGIN metanicator-managed memory-config (do not edit; regenerated by daemon)"
	metanicatorMemoryConfigEndMarker    = "# END metanicator-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(metanicatorMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(metanicatorMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(metanicatorMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(metanicatorMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(MetanicatorCodexMemoryEnv))
...
// renderMetanicatorMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderMetanicatorMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(metanicatorMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(metanicatorMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderMetanicatorMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderMetanicatorMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(metanicatorMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(metanicatorMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderMetanicatorMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderMetanicatorMemoryConfigBlock(true))
}
...
//
// When METANICATOR_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per METANICATOR_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderMetanicatorMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderMetanicatorMemoryConfigBlock(false))
	}
