...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Metanicator-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN METANICATOR-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END METANICATOR-RUNTIME -->"
...
// writeRuntimeConfigFile writes the Metanicator runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Metanicator marker
...
//
//   - User content carries a stray `<!-- END METANICATOR-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Metanicator marker block from the runtime
...
// of Metanicator runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Metanicator task would pick up
...
// agent about the Metanicator runtime environment and available CLI tools.
//
