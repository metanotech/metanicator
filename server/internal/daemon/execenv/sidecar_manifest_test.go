...
// nothing related to Metanicator before a task ran must contain nothing
// related to Metanicator after the task finishes — no .agent_context/,
// no .claude/skills/, no .metanicator/, no stub directories. The test
...
// must still exist and the Metanicator-written sibling must be gone.
...
// round-trip to the .metanicator/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .metanicator/sidecar.txt and created
	// .metanicator/ + .metanicator/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .metanicator/.
	managedDir := filepath.Join(workDir, ".metanicator")
	...
	// .metanicator still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-metanicator/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Metanicator would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Metanicator skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Metanicator
// sibling (which lives at `<slug>-metanicator`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Metanicator's version",
						Content:     "---\nname: issue-review\n---\n\nMetanicator skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Metanicator-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.metanicator/project/
// resources.json` — the other Metanicator-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".metanicator", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".metanicator", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-metanicator`, then `-metanicator-2`,
// `-metanicator-3`, … The PR-review concern is "Metanicator skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-metanicator`.
	...
	if slug != "issue-review-metanicator" {
		t.Errorf("second allocation should bump to `-metanicator`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-metanicator") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-metanicator"), 0o755); err != nil {
	...
	if slug != "issue-review-metanicator-2" {
		t.Errorf("third allocation should be `-metanicator-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-metanicator-2") {
...
// `Issue Review` skill, the Metanicator sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Metanicator sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Metanicator skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Metanicator's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	metanicatorDir := filepath.Join(workDir, ".claude", "skills", "issue-review-metanicator")
	if _, err := os.Stat(filepath.Join(metanicatorDir, "SKILL.md")); err != nil {
		t.Errorf("Metanicator sibling skill should exist at %s: %v", metanicatorDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Metanicator sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(metanicatorDir); !os.IsNotExist(err) {
		t.Errorf("Metanicator sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
