...
// daemon-spawned Codex process can have METANICATOR_TOKEN while `metanicator issue ...`
...
const (
	...
	metanicatorShellEnvBeginMarker = "# BEGIN metanicator-managed shell-environment (do not edit; regenerated by daemon)"
	metanicatorShellEnvEndMarker   = "# END metanicator-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(metanicatorShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(metanicatorShellEnvEndMarker) + `\n*`)
...
// custom_env. Inherited METANICATOR_* variables are always dropped because they
// belong to the daemon process, not necessarily to this task. Explicit
// METANICATOR_* values are safe to include because daemon.go blocklists that
...
func renderMetanicatorShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(metanicatorShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(metanicatorShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderMetanicatorShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
