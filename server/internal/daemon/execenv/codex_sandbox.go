...
// `no such host` errors when calling out (for example, `metanicator issue get`
// hitting the Metanicator API). See upstream issue openai/codex#10390.
//
...
// actually reach the Metanicator API.
//
...
//     rejects non-safe mutation commands "by policy" — e.g. `metanicator issue
...
//     danger-full-access so the Metanicator CLI can reach the API.
...
// metanicatorManagedBeginMarker / metanicatorManagedEndMarker delimit the block the
...
const (
	metanicatorManagedBeginMarker = "# BEGIN metanicator-managed (do not edit; regenerated by daemon)"
	metanicatorManagedEndMarker   = "# END metanicator-managed"
)

// renderMetanicatorManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.metanicator]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertMetanicatorManagedBlock), avoids both traps.
func renderMetanicatorManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(metanicatorManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(metanicatorManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(metanicatorManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(metanicatorManagedEndMarker) + `\n*`)

// upsertMetanicatorManagedBlock returns the config content with the metanicator-managed
...
// `[permissions.metanicator]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderMetanicatorManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertMetanicatorManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderMetanicatorManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the metanicator-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertMetanicatorManagedBlock(existing, policy)
