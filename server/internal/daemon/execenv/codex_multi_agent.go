...
// subagents. The Metanicator daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Metanicator runtime
...
//
// Users who explicitly want Codex native subagents inside a Metanicator task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `METANICATOR_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// MetanicatorCodexMultiAgentEnv is the env var users can set to keep Codex
...
const MetanicatorCodexMultiAgentEnv = "METANICATOR_CODEX_MULTI_AGENT"

// metanicatorMultiAgentBeginMarker / metanicatorMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	metanicatorMultiAgentBeginMarker = "# BEGIN metanicator-managed multi-agent (do not edit; regenerated by daemon)"
	metanicatorMultiAgentEndMarker   = "# END metanicator-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(metanicatorMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(metanicatorMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(MetanicatorCodexMultiAgentEnv))
...
// renderMetanicatorMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderMetanicatorMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(metanicatorMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(metanicatorMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderMetanicatorMultiAgentBlock(true)
...
//
// When METANICATOR_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per METANICATOR_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderMetanicatorMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
