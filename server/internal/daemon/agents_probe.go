...
// /health.agents reports and what `metanicator daemon probe-runtimes` prints.
//
...
// Everything it reads is process-external (PATH, METANICATOR_*_PATH, METANICATOR_*_MODEL),
...
var probeAgentCLIs = func() map[string]AgentEntry {
	...
	// pinned to an explicit METANICATOR_*_PATH) must not pay the cost of
	...
	probe := func(envVar, defaultCmd, modelEnv string) (AgentEntry, bool) {
		...
		// who pinned METANICATOR_*_PATH to an absolute or relative path that
	...
	if e, ok := probe("METANICATOR_CLAUDE_PATH", "claude", "METANICATOR_CLAUDE_MODEL"); ok {
		agents["claude"] = e
	}
	if e, ok := probe("METANICATOR_CODEX_PATH", "codex", "METANICATOR_CODEX_MODEL"); ok {
		agents["codex"] = e
	}
	if e, ok := probe("METANICATOR_OPENCODE_PATH", "opencode", "METANICATOR_OPENCODE_MODEL"); ok {
		agents["opencode"] = e
	}
	if e, ok := probe("METANICATOR_DEVECO_PATH", "deveco", "METANICATOR_DEVECO_MODEL"); ok {
		agents["deveco"] = e
	}
	if e, ok := probe("METANICATOR_OPENCLAW_PATH", "openclaw", "METANICATOR_OPENCLAW_MODEL"); ok {
		agents["openclaw"] = e
	}
	if e, ok := probe("METANICATOR_HERMES_PATH", "hermes", "METANICATOR_HERMES_MODEL"); ok {
		agents["hermes"] = e
	}
	if e, ok := probe("METANICATOR_PI_PATH", "pi", "METANICATOR_PI_MODEL"); ok {
		agents["pi"] = e
	}
	if e, ok := probe("METANICATOR_CURSOR_PATH", "cursor-agent", "METANICATOR_CURSOR_MODEL"); ok {
		agents["cursor"] = e
	}
	if e, ok := probe("METANICATOR_COPILOT_PATH", "copilot", "METANICATOR_COPILOT_MODEL"); ok {
		agents["copilot"] = e
	}
	if e, ok := probe("METANICATOR_KIMI_PATH", "kimi", "METANICATOR_KIMI_MODEL"); ok {
		agents["kimi"] = e
	}
	if e, ok := probe("METANICATOR_REASONIX_PATH", "reasonix", "METANICATOR_REASONIX_MODEL"); ok {
		agents["reasonix"] = e
	}
	if e, ok := probe("METANICATOR_KIRO_PATH", "kiro-cli", "METANICATOR_KIRO_MODEL"); ok {
		agents["kiro"] = e
	}
	if e, ok := probe("METANICATOR_CODEBUDDY_PATH", "codebuddy", "METANICATOR_CODEBUDDY_MODEL"); ok {
		agents["codebuddy"] = e
	}
	// agy 1.0.6 added a `--model` flag (MUL-3125), so Antigravity now takes a
	// model env like every other backend. METANICATOR_ANTIGRAVITY_MODEL seeds the
	...
	if e, ok := probe("METANICATOR_ANTIGRAVITY_PATH", "agy", "METANICATOR_ANTIGRAVITY_MODEL"); ok {
	...
	if e, ok := probe("METANICATOR_QODER_PATH", "qodercli", "METANICATOR_QODER_MODEL"); ok {
		agents["qoder"] = e
	}
	...
	if e, ok := probe("METANICATOR_QODERCLICN_PATH", "qoderclicn", "METANICATOR_QODERCLICN_MODEL"); ok {
		agents["qoderclicn"] = e
	}
	// ByteDance official TRAE CLI (the `traecli` binary from https://docs.trae.cn/cli),
	// driven over ACP via `traecli acp serve --yolo`. METANICATOR_TRAECLI_MODEL seeds
	// the daemon-wide default model (a model id from the user's logged-in traecli
	// catalog).
	if e, ok := probe("METANICATOR_TRAECLI_PATH", "traecli", "METANICATOR_TRAECLI_MODEL"); ok {
		agents["traecli"] = e
	}
	// xAI Grok Build CLI (`grok`), driven over ACP via
	// `grok agent --always-approve stdio`. METANICATOR_GROK_MODEL seeds the
	// daemon-wide default (e.g. grok-4.5).
	if e, ok := probe("METANICATOR_GROK_PATH", "grok", "METANICATOR_GROK_MODEL"); ok {
		agents["grok"] = e
	}
	...
	if e, ok := probe("METANICATOR_QWEN_PATH", "qwen", "METANICATOR_QWEN_MODEL"); ok {
		agents["qwen"] = e
	}
	// QwenPaw (`qwenpaw`) is the QwenPaw CLI agent, driven over ACP via
	// `qwenpaw acp serve`. METANICATOR_QWENPAW_MODEL seeds the daemon-wide default.
	if e, ok := probe("METANICATOR_QWENPAW_PATH", "qwenpaw", "METANICATOR_QWENPAW_MODEL"); ok {
		agents["qwenpaw"] = e
	}
	return agents
}
