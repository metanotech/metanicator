...
// same machine shares one UUID stored at `~/.metanicator/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.metanicator/profiles/*/daemon.id` and returns every
...
// explicitly pins METANICATOR_DAEMON_ID to the hostname itself, there's nothing
