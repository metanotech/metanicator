...
import (
	...
	agentpkg "github.com/metanotech/metanicator/server/pkg/agent"
...
// translates the prompt into a `metanicator issue create` invocation in the
...
// the agent's `metanicator issue create` invocation passes `--project <uuid>`
...
// metadata column is JSONB on the wire; the daemon stores the metanicator CLI
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	// count match reality. See metanicator-ai/metanicator#1660.
