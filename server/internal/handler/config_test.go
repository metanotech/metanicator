...
func TestGetConfigIncludesRuntimeAuthConfig(t *testing.T) {
	...
	t.Setenv("METANICATOR_PUBLIC_URL", "https://api.example.com/")
	t.Setenv("METANICATOR_APP_URL", "https://app.example.com/")
...
func TestGetConfigUsesAppURLForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("METANICATOR_PUBLIC_URL", "")
	t.Setenv("METANICATOR_APP_URL", "https://metanicator.internal.example/")
	...
	if cfg.DaemonServerURL != "https://metanicator.internal.example" {
		t.Fatalf("daemon_server_url: want same-origin URL, got %q", cfg.DaemonServerURL)
	}
	if cfg.DaemonAppURL != "https://metanicator.internal.example" {
...
func TestGetConfigUsesFrontendOriginForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("METANICATOR_PUBLIC_URL", "")
	t.Setenv("METANICATOR_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://metanicator.internal.example/")
...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	t.Setenv("METANICATOR_PUBLIC_URL", "https://api.metanicator.ai")
	t.Setenv("METANICATOR_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://metanicator.ai")
...
// frontend is metanicator.ai, but the deployment does not set METANICATOR_PUBLIC_URL to
// the api host. Previously this fell through to the same-origin branch and
// emitted daemon_server_url=https://metanicator.ai, which the dialog turned into
// `metanicator setup self-host --server-url https://metanicator.ai` — pointing the
...
// setup URLs are omitted and the dialog falls back to `metanicator setup`.
...
// cloud frontend when it is configured through METANICATOR_APP_URL.
func TestGetConfigOmitsCloudDaemonSetupForConfiguredAppURL(t *testing.T) {
	t.Setenv("METANICATOR_PUBLIC_URL", "")
	t.Setenv("METANICATOR_APP_URL", "https://metanicator.ai")
...
func TestURLHostEqualsCanonicalizesCommonHostForms(t *testing.T) {
	tests := []struct {
		...
		{name: "full URL", raw: "https://api.metanicator.ai", want: true},
		{name: "bare host", raw: "api.metanicator.ai", want: true},
		{name: "host port", raw: "api.metanicator.ai:8080", want: true},
		{name: "trailing dot", raw: "https://api.metanicator.ai.", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlHostEquals(tt.raw, "api.metanicator.ai"); got != tt.want {
...
func TestGetConfigExposesServerVersion(t *testing.T) {
	...
	t.Setenv("METANICATOR_APP_URL", "https://metanicator.self-hosted.example")
	t.Setenv("FRONTEND_ORIGIN", "")
...
// suppressed on the managed cloud (frontend host metanicator.ai) even when the
...
func TestGetConfigOmitsServerVersionOnOfficialCloud(t *testing.T) {
	...
	// Official cloud: frontend host metanicator.ai -> version omitted.
	t.Setenv("METANICATOR_APP_URL", "https://metanicator.ai")
	t.Setenv("FRONTEND_ORIGIN", "")
	w := httptest.NewRecorder()
	testHandler.GetConfig(w, req)
