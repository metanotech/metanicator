...
// channel table-of-contents (`metanicator chat history`), Thread reads one thread's
// messages (`metanicator chat thread [id]`). Both are scoped server-side to the
...
// GetChatChannelHistory serves `metanicator chat history` — the channel overview:
// recent top-level messages, each thread tagged with its id + reply count (no
// thread contents). The agent drills into a thread with `metanicator chat thread`.
...
// GetChatThread serves `metanicator chat thread [id]` — one thread's messages. With
