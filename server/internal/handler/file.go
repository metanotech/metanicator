...
type AttachmentResponse struct {
	...
	// METANICATOR_LOCAL_UPLOAD_BASE_URL), and otherwise prefixing
	// METANICATOR_PUBLIC_URL onto the stable per-attachment endpoint that the
...
//     proxies on every request, anchored on `METANICATOR_PUBLIC_URL` so the
//     persisted URL keeps working for clients that don't share the
//     document origin (Desktop / mobile webview).
//
//  3. Last-resort fallback (no `METANICATOR_PUBLIC_URL` configured): emit
