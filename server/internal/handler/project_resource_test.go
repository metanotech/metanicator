...
func TestProjectResourceLifecycle(t *testing.T) {
	...
	if ref.URL != "https://github.com/metanotech/metanicator" {
...
func TestProjectResourceAcceptsSSHRepoURLs(t *testing.T) {
	...
	cases := []struct {
		...
		{"scp-like", "git@github.com:metanicator-ai/metanicator.git"},
		{"ssh-scheme", "ssh://git@github.com/metanotech/metanicator.git"},
	}
...
func TestIsValidGitRepoURL(t *testing.T) {
	good := []string{
		...
		"https://github.com/metanotech/metanicator.git",
		"http://github.example.com/x/y",
		"ssh://git@github.com/metanotech/metanicator.git",
		"ssh://git@github.com:22/metanicator-ai/metanicator.git",
		"git@github.com:metanicator-ai/metanicator.git",
		"git@gitlab.example.com:group/sub/repo.git",
	}
	bad := []string{
		"",
		"not-a-url",
		"github.com/metanotech/metanicator", // no scheme, no scp-style colon
		"https://",                      // empty host
...
func TestCreateProjectAttachesResources(t *testing.T) {
	...
	req := newRequest("POST", "/api/projects?workspace_id="+testWorkspaceID, map[string]any{
		...
		"resources": []map[string]any{
			{
				"resource_type": "github_repo",
				"resource_ref":  map[string]any{"url": "https://github.com/metanotech/metanicator"},
			},
		},
	})
	testHandler.CreateProject(w, req)
...
func TestProjectResourceCountBreadcrumb(t *testing.T) {
	...
	req = newRequest("POST", "/api/projects/"+project.ID+"/resources", map[string]any{
		"resource_type": "github_repo",
		"resource_ref":  map[string]any{"url": "https://github.com/metanotech/breadcrumb"},
	})
	req = withURLParam(req, "id", project.ID)
	testHandler.CreateProjectResource(w, req)
	if w.Code != http.StatusCreated {
...
func TestCreateProjectWithResourcesEchoesCount(t *testing.T) {
	...
	req := newRequest("POST", "/api/projects?workspace_id="+testWorkspaceID, map[string]any{
		...
		"resources": []map[string]any{
			{
				"resource_type": "github_repo",
				"resource_ref":  map[string]any{"url": "https://github.com/metanotech/echo-count"},
			},
		},
	})
	testHandler.CreateProject(w, req)
