...
const devVerificationCodeEnv = "METANICATOR_DEV_VERIFICATION_CODE"
...
func signupSourceFromRequest(r *http.Request) string {
	c, err := r.Cookie("metanicator_signup_source")
	if err != nil || c == nil {
		return ""
	}
...
func (h *Handler) checkSignupAllowed(ctx context.Context, email string, isNewUser bool) error {
	...
	// 3. pending workspace invitation always wins (invitation-based signup)
	if ctx != nil && h.Queries != nil {
		invs, err := h.Queries.ListPendingInvitationsForUser(ctx, db.ListPendingInvitationsForUserParams{
			InviteeEmail: email,
		})
