...
// Set by the metanicator binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Metanicator server API.
...
// reading a capped slice of the body. Every Metanicator API helper funnels its
...
// It can be overridden with the METANICATOR_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring METANICATOR_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("METANICATOR_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors METANICATOR_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects METANICATOR_HTTP_TIMEOUT.
...
// CLI never hands an attachment URL to a native loader — `metanicator attachment
