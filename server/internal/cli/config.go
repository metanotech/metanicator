...
const defaultCLIConfigPath = ".metanicator/config.json"
...
type CLIConfig struct {
	...
	// METANICATOR_DAEMON_DEVICE_NAME env, this field, os.Hostname().
	...
	// METANICATOR_AGENT_RUNTIME_NAME env, this field, the built-in
	// DefaultRuntimeName.
	...
	// METANICATOR_DAEMON_MAX_CONCURRENT_TASKS env, this field, default.
	...
	// METANICATOR_DAEMON_POLL_INTERVAL env, this field, DefaultPollInterval.
	...
	// flag, METANICATOR_DAEMON_HEARTBEAT_INTERVAL env, this field,
	...
	// explicit 0), METANICATOR_AGENT_TIMEOUT env, this field,
	// DefaultAgentTimeout.
	...
	// METANICATOR_CODEX_SEMANTIC_INACTIVITY_TIMEOUT env, this field,
	...
	// METANICATOR_CODEX_HANDSHAKE_TIMEOUT env, this field,
	...
	// already resolves to enabled on Metanicator Cloud. Absent / false means
	// "let env/default decide". Resolution precedence:
	// --no-auto-update flag, METANICATOR_DAEMON_AUTO_UPDATE=false env, this
	...
	// --auto-update-interval flag, METANICATOR_DAEMON_AUTO_UPDATE_INTERVAL
	...
	// metanicator binary that was replaced on disk out of band. Single-direction
	...
	// --no-auto-reload flag, METANICATOR_DAEMON_AUTO_RELOAD=false env, this
	...
	// `metanicator runtime profile set-path`; the daemon prefers it over the
...
//
//	BinaryPath: METANICATOR_OPENCLAW_PATH (env)  > backends.openclaw.binary_path > PATH lookup
...
// METANICATOR_OPENCLAW_STATE_DIR. Rationale: OpenClaw already honors its own env
...
// OpenClaw's own env, METANICATOR_OPENCLAW_STATE_DIR can be layered on top
...
// An empty profile returns the default path (~/.metanicator/config.json).
// A named profile returns ~/.metanicator/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".metanicator", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.metanicator/. A named profile returns ~/.metanicator/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".metanicator"), nil
	}
	return filepath.Join(home, ".metanicator", "profiles", profile), nil
}
