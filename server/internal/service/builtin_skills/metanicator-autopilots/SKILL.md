---
name: metanicator-autopilots
description: "Use when creating, updating, inspecting, triggering, or debugging a Metanicator autopilot (scheduled, webhook, or manual)."
user-invocable: false
allowed-tools: Bash(metanicator *)
---

# Metanicator Autopilots
...
```bash
metanicator autopilot list --output json
metanicator autopilot get <autopilot-id> --output json
metanicator autopilot runs <autopilot-id> --output json
```
...
- `create_issue` creates a Metanicator issue, making the run visible as issue state.
...
metanicator autopilot list --output json
metanicator autopilot get <autopilot-id> --output json
metanicator autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
metanicator autopilot update <autopilot-id> --status active|paused --output json
metanicator autopilot runs <autopilot-id> --output json
metanicator autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
metanicator autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
metanicator autopilot trigger <autopilot-id> --output json
metanicator autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `metanicator autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `metanicator autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `metanicator squad get <squad-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `metanicator agent get <agent-id> --output json` and `metanicator runtime list --output json`.
