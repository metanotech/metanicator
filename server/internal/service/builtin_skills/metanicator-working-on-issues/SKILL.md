---
name: metanicator-working-on-issues
description: "Use when acting on a Metanicator issue beyond what the brief covers: PR linking vs close intent, reading a linked PR's real state, metadata keys, status-change side effects, sub-issue todo vs backlog."
user-invocable: false
allowed-tools: Bash(metanicator *), Bash(git *), Bash(gh *)
---

# Working on Metanicator issues
...
For building mention links, load `metanicator-mentioning` instead — not this skill.
...
This is the link that `metanicator issue pull-requests` reads back — but see the
...
**excluded from `metanicator issue pull-requests`** (and the issue's right-side PR
...
is to open or update a PR before posting the final Metanicator issue comment, unless
...
When a step depends on PR state, query Metanicator's link table — do not infer it
from branch names, GitHub search, memory, or `pr_url` metadata (which can be
stale).

```bash
metanicator issue pull-requests <issue-id> --output json
```
...
```bash
metanicator issue metadata set <issue-id> --key <key> --value <value>
metanicator issue metadata delete <issue-id> --key <stale-key>
```
...
- Read what exists before writing: `metanicator property list` shows the catalog;
  `metanicator issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
metanicator issue property set <issue-id> --name Environment --value staging
metanicator issue property set <issue-id> --name Platforms --value "iOS,Android"
metanicator issue property unset <issue-id> --name Environment
```
...
```bash
metanicator issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
metanicator issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
metanicator issue status <child-id> todo   # promote when the previous step is truly done
```
...
metanicator issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
metanicator issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
metanicator issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
metanicator issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
metanicator issue children <parent-id>             # sub-issues grouped by stage
metanicator issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
metanicator issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
metanicator issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
metanicator issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
metanicator issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
metanicator issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
