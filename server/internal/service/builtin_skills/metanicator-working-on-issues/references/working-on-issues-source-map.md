...
## `metanicator issue pull-requests` — read PR links from Metanicator

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/metanicator/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/metanicator/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/metanicator/cmd_issue.go:522` | `:522` (unchanged) |
...
This is what `metanicator issue pull-requests` later reads back.
...
| `--stage` on `issue create` / `issue update` | `server/cmd/metanicator/cmd_issue.go:328,350` |
| `metanicator issue children <id>` (sub-issues grouped by stage) | `server/cmd/metanicator/cmd_issue.go:114,678`; route `GET /api/issues/{id}/children` → `ListChildIssues` |
...
comment explicitly asks for `metanicator issue status <parent-id> in_review` —
...
| Behavior | File:line |
|---|---|
| `metanicator issue metadata set <issue-id> --key --value [--type]` | `server/cmd/metanicator/cmd_issue_metadata.go:80,109-111` |
| `metanicator issue metadata delete <issue-id> --key` | `server/cmd/metanicator/cmd_issue_metadata.go:93,113` |
...
| Behavior | File:line |
|---|---|
| `metanicator property list/get/create/update/archive/unarchive` | `server/cmd/metanicator/cmd_property.go` |
| `metanicator issue property list/set/unset` (name→id translation) | `server/cmd/metanicator/cmd_property.go` (`encodeIssuePropertyValue`) |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/metanicator/cmd_issue.go
