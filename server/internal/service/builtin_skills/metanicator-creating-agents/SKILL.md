---
name: metanicator-creating-agents
description: "Use when creating, inspecting, or debugging a Metanicator agent definition via the `metanicator agent` CLI or POST /api/agents. Not for assigning issues to agents that already exist, and not for runtime task prompts."
user-invocable: false
allowed-tools: Bash(metanicator *)
---

# Creating Metanicator agents

This is the contract for Metanicator's agent-creation path: what the create entry
...
```bash
metanicator agent get <agent-id> --output json      # full persisted agent record
metanicator agent skills list <agent-id> --output json   # current skill bindings
metanicator agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
`POST /api/agents` (`metanicator agent create`). At task claim time the daemon
...
`metanicator agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
metanicator agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
metanicator agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
metanicator agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
metanicator agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`metanicator agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`metanicator agent env set`), which carries the same
...
```bash
metanicator agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
metanicator agent update <agent-id> --mcp-config-stdin --output json
metanicator agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `metanicator agent create` — inserts a new agent row.
- `metanicator agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `metanicator agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `metanicator agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
