...
This skill states WHAT a mention link does in the Metanicator backend, traced to
...
at a project (see the metanicator-projects-and-resources skill); everything else in
...
- a person → `metanicator workspace member list --output json` → use `user_id`
- an agent → `metanicator agent list --output json` → use `id`
- a squad  → `metanicator squad list --output json` → use `id`
...
- **A hex-ish but wrong UUID.** A well-formed-looking UUID that no entity owns
  ...
  **So when you see `invocation_not_allowed`, check the UUID against the live
  ...
  `metanicator squad member list <squad-id> --output json` returns the `member_id`
...
- **A private agent you cannot invoke:** blocked — the mention path gates on
  ...
  ungated — see the metanicator-squads skill.)
...
Correct:
  1. `metanicator workspace member list --output json`  → Alice's `user_id` = 7f3a…
