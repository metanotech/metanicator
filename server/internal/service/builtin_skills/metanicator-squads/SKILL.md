---
name: metanicator-squads
description: "Use when creating, inspecting, updating, assigning to, or debugging a Metanicator squad, including how leader routing picks who runs."
...
metanicator squad get <squad-id> --output json
metanicator squad member list <squad-id> --output json
metanicator issue comment list <issue-id> --roots-only --summary --output json
metanicator issue comment list <issue-id> --thread <thread-id> --tail 30 --output json
```
...
```bash
metanicator squad --help
metanicator squad member --help
metanicator issue update --help
...
A Metanicator squad is a workspace routing and coordination object.
...
```bash
metanicator squad list --output json
metanicator squad get <squad-id> --output json
metanicator squad create --name <name> --leader <agent-name-or-id> --output json
metanicator squad update <squad-id> --instructions "<leader coordination policy>" --output json
metanicator squad delete <squad-id>
```
...
```bash
metanicator squad member list <squad-id> --output json
metanicator squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
metanicator squad member remove <squad-id> --member-id <id> --type agent|member
metanicator squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
metanicator squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
metanicator issue get <issue-id> --output json
metanicator issue update <issue-id> --help
metanicator issue comment list <issue-id> --roots-only --summary --output json
metanicator issue comment add <issue-id> --help
```
...
For squad leader tasks, Metanicator appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `metanicator-*` skills are not listed —
...
- recording squad activity with `metanicator squad activity`;
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `metanicator issue status` no.
