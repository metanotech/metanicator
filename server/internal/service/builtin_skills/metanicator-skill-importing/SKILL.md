---
name: metanicator-skill-importing
description: "Use when asked to import or install a specific skill into this Metanicator workspace from a URL or slug. Not for choosing which skill the user needs; never treat a local installer such as `npx skills add` as the final install."
user-invocable: false
allowed-tools: Bash(metanicator *)
---

# Importing skills into Metanicator

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Metanicator workspace.
...
A skill is installed for Metanicator only when it exists in the current workspace's
...
```bash
metanicator skill import --url <url> --output json              # hosted source
metanicator skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Metanicator workspace DB, so Metanicator cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `metanicator skill import --url <url> --output json`:

```bash
metanicator skill import --url clawhub.ai/owner/skill --output json
metanicator skill import --url skills.sh/owner/repo/skill --output json
metanicator skill import --url github.com/owner/repo --output json
metanicator skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
metanicator skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`metanicator skill import --file <path> --output json` imports a skill from a local
...
```bash
metanicator skill import --url <url> --output json
```
...
```bash
metanicator agent skills add <agent-id> --skill-ids <skill-id> --output json
metanicator agent skills list <agent-id> --output json
```

After the final `metanicator agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`metanicator agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`metanicator agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `metanicator skill import --url <url>` is equivalent to
...
metanicator skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
metanicator skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
metanicator skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
metanicator skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
metanicator skill get <skill-id> --output json
```
...
```bash
metanicator skill list --output json
metanicator skill get <skill-id> --output json
```
...
Incorrect (bypasses Metanicator):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Metanicator cannot manage it as a workspace skill.
...
```bash
metanicator skill import --url https://skills.sh/owner/repo/skill --output json
```
