---
name: metanicator-onboarding
description: "Use when a product-authored kickoff starts or resumes Mika's interactive onboarding for a Metanicator workspace. Guide the member from the first introduction to one real, confirmed, issue-based execution and a clear handoff."
...
1. What Metanicator is, in one sentence: a workspace where people and AI agents
...
- **Digest** — "Set up a daily automation that posts a morning summary of
  ...
  - The profile block carries `Member IANA timezone`. When it holds a zone,
    ...
    `metanicator autopilot trigger-add --timezone <IANA>`.
