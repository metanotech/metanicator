---
name: metanicator-projects-and-resources
description: "Use when creating, inspecting, updating, or debugging Metanicator projects and their resources (github_repo, local_directory)."
user-invocable: false
allowed-tools: Bash(metanicator *)
---

# Metanicator Projects and Resources
...
metanicator project get <project-id> --output json
...
A project groups work and carries durable resources. A resource is not just display metadata; it is context later injected into task briefs and `.metanicator/project/resources.json`.

A project's `description` is also durable context: when an issue (or a quick-create task) is bound to a project, the project description is injected into the agent's brief under `## Project Context` and written to `.metanicator/project/resources.json` as `project_description`. Use it for project-wide rules/context that should apply to every task in the project.
...
metanicator project list --output json
metanicator project get <project-id> --output json
metanicator project create --title "<title>" --repo <github-url> --output json
metanicator project create --title "<title>" --start-date 2026-03-01 --due-date 2026-03-31 --output json
metanicator project update <project-id> --title "<title>" --output json
metanicator project update <project-id> --due-date 2026-04-15 --output json
metanicator project update <project-id> --start-date "" --output json   # clear the start date
metanicator project status <project-id> in_progress --output json
metanicator project resource list <project-id> --output json
...
metanicator project resource add <project-id> --type local_directory --local-path <abs-path> --daemon-id <daemon-id> --output json
metanicator project resource update <project-id> <resource-id> --url <new-github-url> --output json
metanicator project resource update <project-id> <resource-id> --ref <branch-or-sha> --output json
metanicator project resource remove <project-id> <resource-id> --output json
```
...
Project resources are durable and affect future tasks. `metanicator repo checkout`
is task-local checkout state.

## Debugging wrong context

1. `metanicator project get <project-id> --output json`.
2. `metanicator project resource list <project-id> --output json`.
