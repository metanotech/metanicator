...
import (
	...
	composiointeg "github.com/metanotech/metanicator/server/internal/integrations/composio"
	"github.com/metanotech/metanicator/server/internal/integrations/dingtalk"
	"github.com/metanotech/metanicator/server/internal/integrations/lark"
	...
	composiosdk "github.com/metanotech/metanicator/server/pkg/composio"
...
// METANICATOR_APP_URL and falls back to FRONTEND_ORIGIN, matching how the backend
// resolves the app URL elsewhere (handler.daemonSetupURLsFromEnv) and the CLI
// login flow (cmd/metanicator tryResolveAppURL). Empty when neither is set.
func appURLFromEnv() string {
	if v := strings.TrimRight(strings.TrimSpace(os.Getenv("METANICATOR_APP_URL")), "/"); v != "" {
		return v
	}
...
// METANICATOR_TRUSTED_PROXIES env var. Invalid entries are dropped with a single
...
func parseTrustedProxies(raw string) []netip.Prefix {
	...
	for _, part := range strings.Split(raw, ",") {
		...
		if err != nil {
			slog.Warn("METANICATOR_TRUSTED_PROXIES: ignoring invalid CIDR",
				"value", s, "error", err)
			continue
		}
		out = append(out, p)
...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	signupConfig := handler.Config{
		AllowSignup:              handler.IsAllowSignupFromEnv(),
		AllowedEmails:            splitAndTrim(os.Getenv("ALLOWED_EMAILS")),
		AllowedEmailDomains:      splitAndTrim(os.Getenv("ALLOWED_EMAIL_DOMAINS")),
		DisableWorkspaceCreation: handler.IsWorkspaceCreationDisabledFromEnv(),
		VCSIntegrationEnabled:    os.Getenv("METANICATOR_VCS_INTEGRATION_ENABLED") == "true",
		PublicURL:                strings.TrimRight(strings.TrimSpace(os.Getenv("METANICATOR_PUBLIC_URL")), "/"),
		TrustedProxies:           parseTrustedProxies(os.Getenv("METANICATOR_TRUSTED_PROXIES")),
		CloudRuntimeFleetURL:     cloudRuntimeFleetURLFromEnv(),
		CloudRuntimeFleetTimeout: envDuration("METANICATOR_CLOUD_FLEET_TIMEOUT", 35*time.Second),
		AttachmentDownloadMode:   os.Getenv("ATTACHMENT_DOWNLOAD_MODE"),
		AttachmentDownloadURLTTL: envDuration("ATTACHMENT_DOWNLOAD_URL_TTL", 30*time.Minute),
		AttachmentFrameAncestors: origins,
		LLMAPIKey:                strings.TrimSpace(os.Getenv("METANICATOR_LLM_API_KEY")),
		LLMBaseURL:               strings.TrimSpace(os.Getenv("METANICATOR_LLM_BASE_URL")),
		LLMDefaultModel:          strings.TrimSpace(os.Getenv("METANICATOR_LLM_DEFAULT_MODEL")),
	...
	if opts.BusinessMetrics != nil {
		...
		// metanicator_cloudruntime_request_* histograms.
	...
	// Lark integration. Only wired when METANICATOR_LARK_SECRET_KEY is set:
	...
	if larkKey, err := secretbox.LoadKey("METANICATOR_LARK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				// METANICATOR_LARK_SECRET_KEY is the operator's opt-in for
				...
				// real Lark traffic can point METANICATOR_LARK_HTTP_BASE_URL
				// at a mock server.
				//
				// METANICATOR_LARK_HTTP_BASE_URL is an OPTIONAL deployment-wide
				...
				larkClient := lark.NewHTTPAPIClient(lark.HTTPClientConfig{
					BaseURL: strings.TrimSpace(os.Getenv("METANICATOR_LARK_HTTP_BASE_URL")),
					Logger:  slog.Default(),
				})
				...
				// to initialize (bad METANICATOR_LARK_CALLBACK_BASE_URL or
				...
				go lark.BackfillRegionFromLegacyOverride(context.Background(), cs,
					strings.TrimSpace(os.Getenv("METANICATOR_LARK_HTTP_BASE_URL")),
					strings.TrimSpace(os.Getenv("METANICATOR_LARK_CALLBACK_BASE_URL")),
					slog.Default())
				...
				// METANICATOR_LARK_REGISTRATION_DOMAIN / _LARK_DOMAIN env
				// vars override the protocol hosts for staging / dev.
				regCfg := lark.RegistrationConfig{
					Domain:     strings.TrimSpace(os.Getenv("METANICATOR_LARK_REGISTRATION_DOMAIN")),
					LarkDomain: strings.TrimSpace(os.Getenv("METANICATOR_LARK_REGISTRATION_LARK_DOMAIN")),
				}
		...
		slog.Info("lark integration disabled (METANICATOR_LARK_SECRET_KEY not set)")
	}

	// Slack integration. Multi-tenant B2 model (MUL-3666): Metanicator hosts ONE
	...
	//   - METANICATOR_SLACK_SECRET_KEY decrypts the per-installation bot token
	...
	//     Slack at all.
	//   - METANICATOR_SLACK_APP_TOKEN is the app-level token (xapp-) authorizing the
	...
	if slackKey, err := secretbox.LoadKey("METANICATOR_SLACK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			// the Slack user to their Metanicator account.
			...
			slackReplier := slack.NewOutboundReplier(slack.OutboundReplierConfig{
				...
				// app URL (METANICATOR_APP_URL ?? FRONTEND_ORIGIN), NOT METANICATOR_PUBLIC_URL
			...
			// On-demand history reader behind the unified `metanicator chat history`
			...
			// and a Metanicator notification when the issue lands.
		...
		slog.Info("slack integration disabled (METANICATOR_SLACK_SECRET_KEY not set)")
	}
	...
	if dingtalkKey, err := secretbox.LoadKey("METANICATOR_DINGTALK_SECRET_KEY"); err == nil {
		...
		slog.Info("dingtalk integration disabled (METANICATOR_DINGTALK_SECRET_KEY not set)")
	}
	...
	// (COMPOSIO_CALLBACK_BASE_URL, falling back to METANICATOR_PUBLIC_URL). The
	...
	if composioAPIKey := strings.TrimSpace(os.Getenv("COMPOSIO_API_KEY")); composioAPIKey != "" {
		if !featureflags.ComposioMCPAppsEnabled(context.Background(), opts.FeatureFlags) {
			...
			if err != nil {
				...
				case callbackBase == "":
					slog.Error("composio: no callback base url (set COMPOSIO_CALLBACK_BASE_URL or METANICATOR_PUBLIC_URL); composio integration disabled")
				default:
	...
	if vcsKey, err := secretbox.LoadKey("METANICATOR_VCS_SECRET_KEY"); err == nil {
		...
		slog.Info("vcs integration disabled (METANICATOR_VCS_SECRET_KEY not set)")
	}
	...
	// Cloud PAT verifier: validates mcn_ tokens against Metanicator Cloud
	...
	// Reuses METANICATOR_CLOUD_FLEET_URL (the same URL the cloud-runtime
	...
	// GitHub App webhook (no Metanicator auth — requests are authenticated via
	// HMAC-SHA256 signature in the handler) and post-install setup callback.
	r.Post("/api/webhooks/github", h.HandleGitHubWebhook)
	r.Get("/api/github/setup", h.GitHubSetupCallback)
	// Slack OAuth callback (no Metanicator auth in the path — it is hit by Slack's
	...
	// VCS webhook for token-based providers (Forgejo / Gitea / GitLab). No Metanicator
	...
	// Stripe webhook (no Metanicator auth — Stripe signs the raw body
	// with a shared secret, the metanicator-cloud upstream verifies. We
	...
	r.Group(func(r chi.Router) {
		...
		// cloud-runtime — metanicator-cloud's Fleet and Billing share
...
// METANICATOR_LARK_CALLBACK_BASE_URL), we log and fall back to the
...
func buildLarkConnector(installSvc *lark.InstallationService, apiClient lark.APIClient) (lark.EventConnector, string) {
	endpointFetcher, err := lark.NewHTTPConnectionTokenFetcher(lark.HTTPConnectionTokenConfig{
		BaseURL: strings.TrimSpace(os.Getenv("METANICATOR_LARK_CALLBACK_BASE_URL")),
		Logger:  slog.Default(),
	})
	if err != nil {
	...
	if proxyURL := strings.TrimSpace(os.Getenv("METANICATOR_LARK_WS_PROXY_URL")); proxyURL != "" {
...
func cloudRuntimeFleetURLFromEnv() string {
	if url := strings.TrimSpace(os.Getenv("METANICATOR_CLOUD_FLEET_URL")); url != "" {
		return url
	}
	return strings.TrimSpace(os.Getenv("METANICATOR_FLEET_URL"))
}
...
// already-resolved METANICATOR_PUBLIC_URL, then the app URL.
