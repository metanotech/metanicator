...
func TestMain(m *testing.M) {
	for _, key := range []string{
		"METANICATOR_AGENT_ID",
		"METANICATOR_TASK_ID",
		"METANICATOR_TOKEN",
		"METANICATOR_DAEMON_PORT",
		"METANICATOR_WORKSPACE_ID",
...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers METANICATOR_APP_URL", func(t *testing.T) {
		t.Setenv("METANICATOR_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://metanicator.ai/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `metanicator login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}

// TestLoginTokenHelpOutputRendersCleanly renders loginCmd's flag help through
// the same pflag path `metanicator login -h` uses (FlagUsagesWrapped) and locks the
...
// `metanicator login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Metanicator Cloud Node PATs (`mcn_`) at
