...
// impersonation escape: a sandbox fault strips every METANICATOR_* env var from
// an agent subprocess, which then runs `metanicator` from the *parent* directory
...
func TestNewAPIClient_WorkdirParentEscapeFailsClosed(t *testing.T) {
	...
	// Daemon-owned tree: {root}/.metanicator marker + {root}/{ws}/{task}/workdir
	...
	t.Setenv("METANICATOR_AGENT_ID", "")
	t.Setenv("METANICATOR_TASK_ID", "")
	t.Setenv("METANICATOR_DAEMON_PORT", "")
	t.Setenv("METANICATOR_TOKEN", "")
	t.Setenv("METANICATOR_SERVER_URL", "http://127.0.0.1:8080")
...
// agent task (METANICATOR_AGENT_ID / METANICATOR_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.metanicator/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("outside agent context falls back to config", func(t *testing.T) {
		...
		t.Setenv("METANICATOR_TASK_ID", "")
		t.Setenv("METANICATOR_DAEMON_PORT", "")
		t.Setenv("METANICATOR_WORKSPACE_ID", "")
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("METANICATOR_AGENT_ID", "agent-123")
		t.Setenv("METANICATOR_TASK_ID", "task-456")
		t.Setenv("METANICATOR_DAEMON_PORT", "")
		t.Setenv("METANICATOR_WORKSPACE_ID", "env-ws")
	...
	t.Run("daemon port marker also skips config", func(t *testing.T) {
		t.Setenv("METANICATOR_AGENT_ID", "")
		t.Setenv("METANICATOR_TASK_ID", "")
		t.Setenv("METANICATOR_DAEMON_PORT", "27182")
		t.Setenv("METANICATOR_WORKSPACE_ID", "")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without METANICATOR_TOKEN", got)
		}
	})
	...
	t.Run("daemon port marker without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in daemon-managed context without METANICATOR_TOKEN", got)
		}
	})
	...
	t.Run("agent context uses explicit task token env", func(t *testing.T) {
		...
		t.Setenv("METANICATOR_TASK_ID", "task-456")
		t.Setenv("METANICATOR_DAEMON_PORT", "")
		t.Setenv("METANICATOR_TOKEN", "mat_task_token")

		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want METANICATOR_TOKEN", got)
		}
	})
	...
	t.Run("daemon port set without agent context avoids config fallback", func(t *testing.T) {
		...
		t.Setenv("METANICATOR_TASK_ID", "")
		t.Setenv("METANICATOR_TOKEN", "")
		t.Setenv("METANICATOR_DAEMON_PORT", "19514")
	...
	t.Run("daemon port set with explicit task token uses task token", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want METANICATOR_TOKEN (task token wins over daemon signal)", got)
		}
	})

	// METANICATOR_SERVER_URL is a user-facing env var that may be set in a
	// normal shell. It is NOT a daemon identity signal — only
	// METANICATOR_DAEMON_PORT is. The config fallback must still work when
	// SERVER_URL is set but no daemon signal is present.
	t.Run("METANICATOR_SERVER_URL alone does not block config fallback", func(t *testing.T) {
		t.Setenv("METANICATOR_AGENT_ID", "")
		t.Setenv("METANICATOR_TASK_ID", "")
		t.Setenv("METANICATOR_TOKEN", "")
		t.Setenv("METANICATOR_DAEMON_PORT", "")
		t.Setenv("METANICATOR_SERVER_URL", "https://api.metanicator.ai")
...
func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("METANICATOR_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("METANICATOR_WORKSPACE_ID", "workspace-123")
	t.Setenv("METANICATOR_AGENT_ID", "agent-123")
	t.Setenv("METANICATOR_TASK_ID", "task-456")
	...
	t.Run("member token fails closed", func(t *testing.T) {
		t.Setenv("METANICATOR_TOKEN", "mul_member_token")
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("METANICATOR_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("METANICATOR_WORKSPACE_ID", "test-ws")
	t.Setenv("METANICATOR_TOKEN", "test-token")
	t.Setenv("METANICATOR_AGENT_ID", "")
	t.Setenv("METANICATOR_TASK_ID", "")
	...
	msg := err.Error()
	if !strings.Contains(msg, "metanicator agent env set") {
		t.Fatalf("no-fields error must direct users to `metanicator agent env set`; got: %q", msg)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `metanicator agent env set` instead", flag)
		}
	}
}
...
func TestAgentMaxConcurrentTasksFlagValidation(t *testing.T) {
	...
	t.Setenv("HOME", t.TempDir())
	t.Setenv("METANICATOR_SERVER_URL", srv.URL)
	t.Setenv("METANICATOR_WORKSPACE_ID", "ws-1")
	t.Setenv("METANICATOR_TOKEN", "test-token")
	t.Setenv("METANICATOR_AGENT_ID", "")
	t.Setenv("METANICATOR_TASK_ID", "")
