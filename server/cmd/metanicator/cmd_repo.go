...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("METANICATOR_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("METANICATOR_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("METANICATOR_WORKSPACE_ID")
	agentName := os.Getenv("METANICATOR_AGENT_NAME")
	taskID := os.Getenv("METANICATOR_TASK_ID")
	...
	reqBody := map[string]string{
		...
		"task_id":       taskID,
		"checkout_mode": strings.TrimSpace(os.Getenv("METANICATOR_REPO_CHECKOUT_MODE")),
	}
