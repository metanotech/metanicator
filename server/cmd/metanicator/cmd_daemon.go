...
import (
	...
	logger_pkg "github.com/metanotech/metanicator/server/internal/logger"
...
var daemonDiskUsageCmd = &cobra.Command{
	...
	Long: "Walks the daemon's workspaces root and reports per-task or per-workspace disk usage.\n" +
		...
		"root — the default root plus each ~/.metanicator/profiles/* root, including the Desktop app's dedicated\n" +
		...
		"overridable via METANICATOR_GC_ARTIFACT_PATTERNS) so the report stays in sync with what the GC reclaims.\n" +
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: METANICATOR_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: METANICATOR_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: METANICATOR_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: METANICATOR_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: METANICATOR_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: METANICATOR_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: METANICATOR_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: METANICATOR_CODEX_HANDSHAKE_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: METANICATOR_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: METANICATOR_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: METANICATOR_DAEMON_AUTO_UPDATE_INTERVAL)")
	f.Bool("no-auto-reload", false, "Disable restarting when the metanicator binary on disk changes version (env: METANICATOR_DAEMON_AUTO_RELOAD=false)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: METANICATOR_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: METANICATOR_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: METANICATOR_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: METANICATOR_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: METANICATOR_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: METANICATOR_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: METANICATOR_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: METANICATOR_CODEX_HANDSHAKE_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: METANICATOR_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: METANICATOR_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: METANICATOR_DAEMON_AUTO_UPDATE_INTERVAL)")
	rf.Bool("no-auto-reload", false, "Disable restarting when the metanicator binary on disk changes version (env: METANICATOR_DAEMON_AUTO_RELOAD=false)")
	...
	df.Bool("all-profiles", false, "Scan every workspace root (default root + all ~/.metanicator/profiles/* roots, incl. the Desktop app's) and report a combined total")
...
// Empty profile → ~/.metanicator/, named profile → ~/.metanicator/profiles/<name>/.
...
func newDaemonLogRotator(logPath string) *lumberjack.Logger {
	return &lumberjack.Logger{
		Filename:   logPath,
		MaxSize:    envPositiveIntOrDefault("METANICATOR_DAEMON_LOG_MAX_SIZE_MB", defaultDaemonLogMaxSizeMB),
		MaxBackups: envPositiveIntOrDefault("METANICATOR_DAEMON_LOG_MAX_BACKUPS", defaultDaemonLogMaxBackups),
		MaxAge:     envPositiveIntOrDefault("METANICATOR_DAEMON_LOG_MAX_AGE_DAYS", defaultDaemonLogMaxAgeDays),
		Compress:   true,
	}
}
...
// requireDaemonAuth fails fast when the user never ran `metanicator login`. The
...
// authenticates via the stored config token, never METANICATOR_TOKEN.
func requireDaemonAuth(profile string) error {
	...
	if cfg.Token == "" {
		...
		if profile != "" {
			loginHint = fmt.Sprintf("metanicator login --profile %s", profile)
		}
...
// METANICATOR_SERVER_URL env, then the stored CLI config for the profile.
func resolveDaemonServerURL(cmd *cobra.Command, profile string) string {
	serverURL := cli.FlagOrEnv(cmd, "server-url", "METANICATOR_SERVER_URL", "")
	if serverURL == "" {
...
func daemonStartupFailureError(logs daemonStartupLogs, waitErr error, profile, serverURL string) error {
	...
	case strings.Contains(joined, "connection refused") ||
		...
		target := "the Metanicator server"
		if serverURL != "" {
			target += " at " + serverURL
		}
		return fmt.Errorf("daemon failed to start: cannot reach %s.\nMake sure the server is running and reachable, then rerun 'metanicator daemon start'.\nFull log: %s", target, logs.logPath)
	}
...
func buildDaemonStartArgs(cmd *cobra.Command) []string {
	...
	// (= no cap), so it can override an environment METANICATOR_AGENT_TIMEOUT.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	//
	//   --flag  >  METANICATOR_… env  >  config.json  >  built-in default
	//
	...
	pollOverride, err := resolveDaemonDurationOverride(pollFlag, "METANICATOR_DAEMON_POLL_INTERVAL", fileCfg.PollInterval)
	if err != nil {
		return err
	}
	if pollOverride > 0 {
	...
	heartbeatOverride, err := resolveDaemonDurationOverride(heartbeatFlag, "METANICATOR_DAEMON_HEARTBEAT_INTERVAL", fileCfg.HeartbeatInterval)
	if err != nil {
		return err
	}
	...
	agentTimeoutOverride, err := resolveDaemonAgentTimeoutOverride(cmd, "METANICATOR_AGENT_TIMEOUT", fileCfg.AgentTimeout)
	if err != nil {
		return err
	}
	...
	semanticOverride, err := resolveDaemonDurationOverride(semanticFlag, "METANICATOR_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", fileCfg.CodexSemanticInactivityTimeout)
	if err != nil {
		return err
	}
	...
	handshakeOverride, err := resolveDaemonDurationOverride(handshakeFlag, "METANICATOR_CODEX_HANDSHAKE_TIMEOUT", fileCfg.CodexHandshakeTimeout)
	if err != nil {
		return err
	}
	...
	if n := resolveDaemonIntOverride(maxFlag, "METANICATOR_DAEMON_MAX_CONCURRENT_TASKS", fileCfg.MaxConcurrentTasks); n > 0 {
		overrides.MaxConcurrentTasks = n
	}
	// --no-auto-update / METANICATOR_DAEMON_AUTO_UPDATE=false / config.json
	...
	if resolveDaemonDisableSignal(noAutoUpdateFlag, "METANICATOR_DAEMON_AUTO_UPDATE", fileCfg.DisableAutoUpdate) {
	...
	if resolveDaemonDisableSignal(noAutoReloadFlag, "METANICATOR_DAEMON_AUTO_RELOAD", fileCfg.DisableAutoReload) {
		overrides.DisableAutoReload = true
	}
	autoUpdateFlag, _ := cmd.Flags().GetDuration("auto-update-interval")
	autoUpdateOverride, err := resolveDaemonDurationOverride(autoUpdateFlag, "METANICATOR_DAEMON_AUTO_UPDATE_INTERVAL", fileCfg.AutoUpdateCheckInterval)
	if err != nil {
		return err
	}
	...
	cfg.LaunchedBy = os.Getenv("METANICATOR_LAUNCHED_BY")
...
// `metanicator auth status`) and refuse to touch the running daemon on failure.
func requireDaemonRestartPreflight(cmd *cobra.Command, profile string) error {
	...
	if err := cli.NewAPIClient(baseURL, "", cfg.Token).GetJSON(ctx, "/api/me", nil); err != nil {
		...
		return fmt.Errorf("refusing to restart: cannot reach the Metanicator server at %s (%w); the running daemon was left untouched.\nMake sure the server is running and reachable, then rerun 'metanicator daemon restart'", baseURL, err)
	}
	return nil
}
...
//  2. METANICATOR_AGENT_TIMEOUT is set -> return nil so LoadConfig reads the
...
//  2. METANICATOR_DAEMON_AUTO_UPDATE explicitly set to a falsy value ->
...
// each ~/.metanicator/profiles/* root) and renders a per-root breakdown with a
...
// orientation even when empty), then each ~/.metanicator/profiles/* root that
// exists on disk, sorted by profile name. Roots that resolve to the same path
// (e.g. when METANICATOR_WORKSPACES_ROOT pins every profile to one directory) are
...
func printRepoCacheLine(w io.Writer, report daemon.DiskUsageReport) {
	...
	fmt.Fprintf(w, "Repo cache (.repos): %s across %d repo(s), not included above. Evicted once a repo is unused for METANICATOR_GC_REPO_TTL and no longer attached to a workspace.\n",
...
func printDiskUsageOtherRootsHint(w io.Writer, report daemon.DiskUsageReport, profile, rootOverride string) {
	...
	fmt.Fprintln(w, "Run 'metanicator daemon disk-usage --all-profiles' for a combined total across all roots.")
}
...
func diskUsageProfileSuggestions(currentProfile, currentRoot string) []diskUsageProfileSuggestion {
		...
		if root, err := daemon.ResolveWorkspacesRoot("", ""); err == nil && !samePath(root, currentRoot) {
			if taskCount := countDiskUsageTaskDirs(root); taskCount > 0 {
				out = append(out, diskUsageProfileSuggestion{
					Profile:   "",
					Command:   "metanicator daemon disk-usage",
					Root:      root,
					TaskCount: taskCount,
				})
			}
	...
	for _, entry := range entries {
		...
		out = append(out, diskUsageProfileSuggestion{
			Profile:   profile,
			Command:   "metanicator --profile " + shellQuoteArg(profile) + " daemon disk-usage",
			Root:      root,
			TaskCount: taskCount,
		})
	}
...
func profilesRootDir() (string, error) {
	...
	return filepath.Join(home, ".metanicator", "profiles"), nil
}
