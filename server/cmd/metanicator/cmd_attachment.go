...
var attachmentDownloadCmd = &cobra.Command{
  ...
  $ metanicator attachment download abc123
  ...
  $ metanicator attachment download abc123 -o ./attachments`,
	Args: exactArgs(1),
...
The task id is read from METANICATOR_TASK_ID (set by the daemon inside a task);
override it with --task when needed.`,
	Example: `  # Attach an image to the current chat reply
  $ metanicator attachment upload ./chart.png`,
	Args: exactArgs(1),
...
func init() {
	...
	attachmentUploadCmd.Flags().String("task", "", "Chat task id to attach to (defaults to METANICATOR_TASK_ID)")
}
...
func runAttachmentUpload(cmd *cobra.Command, args []string) error {
	...
	if taskID == "" {
		return fmt.Errorf("no chat task in context: run inside a chat task (METANICATOR_TASK_ID set) or pass --task <id>")
	}
