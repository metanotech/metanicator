...
func TestRunWorkspaceCreatePostsWorkspaceAndDoesNotSwitchDefault(t *testing.T) {
	...
	t.Setenv("METANICATOR_WORKSPACE_ID", "existing-workspace")
...
func TestRunWorkspaceSwitch(t *testing.T) {
	...
	// Isolate HOME so the test never touches the developer's ~/.metanicator.
	t.Setenv("HOME", t.TempDir())
	t.Setenv("METANICATOR_SERVER_URL", srv.URL)
	t.Setenv("METANICATOR_TOKEN", "test-token")
	...
	t.Run("isolates by profile", func(t *testing.T) {
		...
		wantSuffix := filepath.Join(".metanicator", "profiles", "staging", "config.json")
