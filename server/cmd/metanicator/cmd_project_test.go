...
// review round 2: `metanicator project resource update <p> <r> --default-branch-hint x`
...
func TestBuildResourceRefFromFlagsGithubMergesHint(t *testing.T) {
	t.Run("hint-only edit preserves existing url", func(t *testing.T) {
		...
		existing := map[string]any{"url": "https://github.com/metanotech/metanicator"}
	...
	t.Run("url override survives merge", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/metanotech/new-repo")
		existing := map[string]any{
			"url":                 "https://github.com/metanotech/metanicator",
		...
		if ref["url"] != "https://github.com/metanotech/new-repo" {
...
func TestBuildResourceRefFromRefFlagKeepsJSONEscapeHatch(t *testing.T) {
	t.Run("json payload wins over github shortcuts", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/metanotech/ignored")
		_ = cmd.Flags().Set("ref", `{"url":"https://github.com/metanotech/metanicator","ref":"release/v2"}`)
		...
		if payload["url"] != "https://github.com/metanotech/metanicator" {
	...
	t.Run("broken json is still rejected", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/metanotech/metanicator")
