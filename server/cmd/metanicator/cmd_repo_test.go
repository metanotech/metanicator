...
func TestRunRepoCheckoutForwardsManagedCheckoutMode(t *testing.T) {
	...
	t.Setenv("METANICATOR_DAEMON_PORT", strings.TrimPrefix(srv.URL, "http://127.0.0.1:"))
	t.Setenv("METANICATOR_WORKSPACE_ID", "ws-1")
	t.Setenv("METANICATOR_AGENT_NAME", "Test Agent")
	t.Setenv("METANICATOR_TASK_ID", "task-1")
	t.Setenv("METANICATOR_REPO_CHECKOUT_MODE", "isolated")
