...
var rootCmd = &cobra.Command{
	Use:           "metanicator",
	Short:         "Metanicator CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Metanicator from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("metanicator {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Metanicator server URL (env: METANICATOR_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: METANICATOR_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: METANICATOR_DEBUG)")
