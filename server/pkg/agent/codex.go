...
const (
	metanicatorCodexMcpBeginMarker = "# BEGIN metanicator-managed mcp_servers (do not edit; regenerated by daemon)"
	metanicatorCodexMcpEndMarker   = "# END metanicator-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(metanicatorCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(metanicatorCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = metanicatorCodexMcpBeginMarker + "\n" + metanicatorCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(metanicatorCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(metanicatorCodexMcpEndMarker)
	sb.WriteString("\n")
...
func normalizeCodexMcpServerConfig(server map[string]any) map[string]any {
	if !isCodexRemoteMcpServer(server) {
		...
		for k, v := range server {
			if isMetanicatorMcpSelectorKey(k) {
				continue
			}
			normalized[k] = v
		}
	...
	for k, v := range server {
		switch {
		case isMetanicatorMcpSelectorKey(k):
			continue
		case k == "type":
			continue
		case k == "headers":
...
func isMetanicatorMcpSelectorKey(k string) bool {
	switch k {
