...
not import any other Metanicator package, so it can be reused by other services or
...
link, err := client.CreateLink(ctx, composio.CreateLinkRequest{
    ...
    UserID:       metanicatorUserID.String(), // your own user id
    CallbackURL:  "https://metanicator.ai/api/integrations/composio/callback",
})
...
- **Standalone.** Zero coupling to Metanicator internals — depend on this package
